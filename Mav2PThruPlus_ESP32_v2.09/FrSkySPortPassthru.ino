...
void SendStatusTextChunk5000() {
  ...
  if (fr_chunk_pntr == 0) { 
    ST_Rec = (MsgRingBuff.shift());  // Get a status text message from front of queue
    fr_severity = ST_Rec.severity;
    fr_txtlth = ST_Rec.txtlth;
    memcpy(fr_text, ST_Rec.text, fr_txtlth+4);   // plus rest of last chunk at least
    fr_simple = ST_Rec.simple;
