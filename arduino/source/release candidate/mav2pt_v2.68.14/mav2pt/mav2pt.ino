...
=====================================================================================================================
  ...
  applications, and to alter it and redistribute it freely, subject to the following restriction:
  ...
  * The origin of this software must not be misrepresented; you must not claim that you wrote the original software.
  ...
  By downloading this software you are agreeing to the terms specified in this page and the spirit thereof.
    ...
    Acknowledgement and thanks to the authors of, and contributors to, mavESP8266 serial/Wifi bridge
...
void MarkHome();
uint32_t getVoltAverage1(uint16_t);
uint32_t getVoltAverage2(uint16_t);
uint32_t getCurrentAverage1(uint16_t);
uint32_t getCurrentAverage2(uint16_t); 
...
void nbdelay(uint32_t delaymS); // my no block delay
uint32_t createMask(uint8_t lo, uint8_t hi);
  ...
  #endif
//===================================  
void ARDUINO_ISR_ATTR restartWiFi() 
{
  restartWiFiSta();
}
//=================================================================================================
...
//=================================================================================================
    ...
    log.printf("%dhx%dw  text_size=%d  char_h_px=%d  char_w_px=%d  scr_h_ch=%d  scr_w_ch=%d\n", scr_h_px, scr_w_px, TEXT_SIZE, char_h_px, char_w_px, scr_h_ch, scr_w_ch);
...
//=================================================================================================
  ...
  if (set.fc_io == fc_ser)   {
    log.println("Mavlink FC Serial");
    LogScreenPrintln("Mav FC Serial");
  }

  if (set.fc_io == fc_bt)  {
    log.println("Mavlink FC Bluetooth");
    LogScreenPrintln("Mav FC BT");
  } 

  if (set.fc_io == fc_wifi)  {
    log.println("Mavlink FC WiFi");
    LogScreenPrintln("Mav FC WiFi");
  } 
  ...
  if (set.fr_io == fr_ser)     {         // 1
    log.println("FrSky Serial");
    LogScreenPrintln("Frs Serial");
  } 
  #if (defined wifiBuiltin)     
    else
    if (set.fr_io == fr_udp)  {           // 2
      log.println("FrSky UDP");
      LogScreenPrintln("Frs UDP");
    } else   
    if (set.fr_io == fr_ser_udp)    {     // 3
      log.println("FrSky Serial and UDP");
      LogScreenPrintln("Frs Serial+UDP");
    } 
  #endif
  #if (defined wifiBuiltin) && (defined sdBuiltin)    
    else   
    if (set.fr_io == fr_udp_sd)    {      // 4
      log.println("FrSky UDP and SD");
      LogScreenPrintln("Frs UDP+SD");
    } else    
    if (set.fr_io == fr_ser_sd)    {      // 5
      log.println("FrSky Serial and SD");
      LogScreenPrintln("Frs Serial+SD");
    } else  
    ...
    if (set.fr_io == fr_ser_udp_sd)     { // 7
      log.println("FrSky Serial and UDP and SD");
      LogScreenPrintln("Frs Ser+UDP+SD");
    }
  #endif
  if (set.gs_io == gs_ser)  {
    log.println("Mavlink GCS Serial");
    LogScreenPrintln("Mav GCS Serial");
  }
  #if (defined btBuiltin)   
    if (set.gs_io == gs_bt)  {
      log.println("Mavlink GCS Bluetooth");
      LogScreenPrintln("Mav GCS BT");
    }
    #endif
  #if (defined wifiBuiltin)  
    if (set.gs_io == gs_wifi)  {
      log.println("Mavlink GCS WiFi");
      LogScreenPrintln("Mav GCS WiFi Out");
    }
  #endif
  #if (defined wifiBuiltin) && (defined btBuiltin)    
    if (set.gs_io == gs_wifi_bt)  {
      log.println("Mavlink GCS WiFi+BT");
      LogScreenPrintln("Mav GCS WiFi+BT");
    }
  #endif
  #if defined wifiBuiltin  
  ...
  if ((set.fc_io == fc_ser) || (set.gs_io == gs_ser))  {  //  Serial 
      ...
      if (fc_rxPin != -1) {
        ...
        delay(100);
          ...
          if (fc_txPin != -1) {     
            fcSerial.begin(set.baud, SWSERIAL_8N1, fc_rxPin, fc_txPin);         
            log.printf("Mavlink FC softwareserial on pins rx:%d and tx:%d  baud:%d\n", fc_rxPin, fc_txPin, set.baud);
          }
          ...
          if (fc_txPin != -1) {
            ...
            log.printf("Mavlink FC serial on pins rx:%d and tx:%d  baud:%d\n", fc_rxPin, fc_txPin, set.baud);
          }
        #endif
        delay(100);
        
        #if (defined ESP32_GCS_SWSERIAL) 
          if (gs_txPin != -1) {     
            gsSerial.begin(set.baud, SWSERIAL_8N1, gs_rxPin, gs_txPin);           
            log.printf("Mavlink GCS softwareserial on pins rx:%d and tx:%d  baud:%d\n", fc_rxPin, fc_txPin, set.baud);
          }
        #else  // ESP32 HardwareSerial
          // system can wait here for a few seconds (timeout) if there is no telemetry in
          if (gs_txPin != -1) {
            gsSerial.begin(set.baud, SERIAL_8N1, gs_rxPin, gs_txPin);   //  rx,tx, cts, rts  
            log.printf("Mavlink GCS serial on pins rx:%d and tx:%d  baud:%d\n", fc_rxPin, fc_txPin, set.baud);
          }
        #endif
        delay(100);
...
void DecodeOneMavFrame() {
   ...
   switch(R2Gmsg.msgid) {
        ...
        case MAVLINK_MSG_ID_SYS_STATUS:   // #1
          ...
          ap_voltage_battery1 = getVoltAverage1(mavlink_msg_sys_status_get_voltage_battery(&R2Gmsg));        // V  from getVoltAverage1()
          ap_current_battery1 = getCurrentAverage1(mavlink_msg_sys_status_get_current_battery(&R2Gmsg));     // dA,  100 = 1A
     ...
     // mav2
        ...
        case MAVLINK_MSG_ID_BATTERY2:          // #181   https://mavlink.io/en/messages/ardupilotmega.html
          if (!mavGood) break;
          ap_voltage_battery2 = getVoltAverage2(mavlink_msg_battery2_get_voltage(&R2Gmsg));        // 1000 = 1V
          ap_current_battery2 = getCurrentAverage2(mavlink_msg_battery2_get_current_battery(&R2Gmsg));     //  100 = 1A
