...
//=================================================================================================  
  ...
  //===============================       H a n d l e   W i F i   E v e n t s 
   ...
   void checkStaLinkStatus() {
      ...
      if ( !WiFi.isConnected() )   
      { 
        if (!wifiDisconnected) startWiFiTimer();   // start wifi retry 
        wifiDisconnected = true;
        #if (defined ESP32) || (defined ESP8266)
          if ( (wifi_retry_millis > 0) && ( (millis() - wifi_retry_millis) > 5000) ) 
          {
            restartWiFiSta();
     ...
     if (!apFailover) {   
      ...
      if (set.wfmode == ap_sta) {
        if (WiFi.mode(WIFI_AP_STA)) {
           log.println("WiFi mode set to AP_STA successfully"); 
        } else {
          log.println("WiFi mode set to AP_STA failed!");  
        }
      } else {
        if (WiFi.mode(WIFI_STA)) {
           log.println("WiFi mode set to STA successfully");  
        } else {
...
//=================================================================================================
uint32_t getVoltAverage1(uint16_t mV)  {
...
//=================================================================================================  
uint32_t getCurrentAverage1(uint16_t dA)  {   // in 10*milliamperes (1 = 10 milliampere)
...
//=================================================================================================  
uint32_t getVoltAverage2(uint16_t mV)  {
...
uint32_t getCurrentAverage2(uint16_t dA)  {
...
void WiFiEvent(WiFiEvent_t event) {
  ...
  switch (event) {
    case ARDUINO_EVENT_WIFI_READY:               log.println("WiFi interface ready"); break;
    case ARDUINO_EVENT_WIFI_SCAN_DONE:           log.println("Completed scan for access points"); break;
    case ARDUINO_EVENT_WIFI_STA_START:           log.println("WiFi client started"); break;
    case ARDUINO_EVENT_WIFI_STA_STOP:            log.println("WiFi clients stopped"); break;
    case ARDUINO_EVENT_WIFI_STA_CONNECTED:       log.println("Connected to access point"); break;
    case ARDUINO_EVENT_WIFI_STA_DISCONNECTED:    log.println("Disconnected from WiFi access point"); break;
    case ARDUINO_EVENT_WIFI_STA_AUTHMODE_CHANGE: log.println("Authentication mode of access point has changed"); break;
    case ARDUINO_EVENT_WIFI_STA_GOT_IP:
      log.print("Obtained IP address: ");
      log.println(WiFi.localIP());
      break;
    case ARDUINO_EVENT_WIFI_STA_LOST_IP:        log.println("Lost IP address and IP address is reset to 0"); break;
    case ARDUINO_EVENT_WPS_ER_SUCCESS:          log.println("WiFi Protected Setup (WPS): succeeded in enrollee mode"); break;
    case ARDUINO_EVENT_WPS_ER_FAILED:           log.println("WiFi Protected Setup (WPS): failed in enrollee mode"); break;
    case ARDUINO_EVENT_WPS_ER_TIMEOUT:          log.println("WiFi Protected Setup (WPS): timeout in enrollee mode"); break;
    case ARDUINO_EVENT_WPS_ER_PIN:              log.println("WiFi Protected Setup (WPS): pin code in enrollee mode"); break;
    case ARDUINO_EVENT_WIFI_AP_START:           log.println("WiFi access point started"); break;
    case ARDUINO_EVENT_WIFI_AP_STOP:            log.println("WiFi access point  stopped"); break;
    case ARDUINO_EVENT_WIFI_AP_STACONNECTED:    log.println("Client connected"); break;
    case ARDUINO_EVENT_WIFI_AP_STADISCONNECTED: log.println("Client disconnected"); break;
    case ARDUINO_EVENT_WIFI_AP_STAIPASSIGNED:   log.println("Assigned IP address to client"); break;
    case ARDUINO_EVENT_WIFI_AP_PROBEREQRECVED:  log.println("Received probe request"); break;
    case ARDUINO_EVENT_WIFI_AP_GOT_IP6:         log.println("AP IPv6 is preferred"); break;
    case ARDUINO_EVENT_WIFI_STA_GOT_IP6:        log.println("STA IPv6 is preferred"); break;
    case ARDUINO_EVENT_ETH_GOT_IP6:             log.println("Ethernet IPv6 is preferred"); break;
    case ARDUINO_EVENT_ETH_START:               log.println("Ethernet started"); break;
    case ARDUINO_EVENT_ETH_STOP:                log.println("Ethernet stopped"); break;
    case ARDUINO_EVENT_ETH_CONNECTED:           log.println("Ethernet connected"); break;
    case ARDUINO_EVENT_ETH_DISCONNECTED:        log.println("Ethernet disconnected"); break;
    case ARDUINO_EVENT_ETH_GOT_IP:              log.println("Obtained IP address"); break;
    default:                                    break;
  }
}
#endif
...
//================================================================================================= 
    ...
    void PaintLogScreen(uint8_t new_row, last_row_t last_row_action) { 
      ...
      #if (defined ST7789_DISPLAY) || (defined ST7789V_DISPLAY) || (defined SSD1331_DISPLAY) ||  (defined ILI9341_DISPLAY)   
  ...
  #endif  // end of defined DISPLAY_PRESENT
    ...
    void DisplayFlightInfo() {
      if (pt_rssi <= Rssi_Blink_threshold)
      {
        displayBlink();
      }
      uint16_t xx, yy; 
      if (display_mode != flight_info) {
        ...
        if (millis() - info_millis > 2000) {    // refresh rate
          ...
          xx = 0;
            ...
            yy = 1.1 * char_h_px;            
          #elif (defined ST7789_DISPLAY) || (defined ST7789V_DISPLAY)   
            yy = 1.8 * char_h_px;  
          #endif  
          display.setCursor(xx, yy);            
    ...
    void setupLogDisplayStyle() {
      ...
      #if (defined ST7789_DISPLAY) || (defined ST7789V_DISPLAY)  // LILYGO® TTGO T-Display Colour LCD
   ...
   //===================================
    ...
    void setupInfoDisplayStyle() {
      ...
      #if (defined ST7789_DISPLAY) || (defined ST7789V_DISPLAY)  // LILYGO® TTGO T-Display Colour LCD    // LILYGO® TTGO T-Display ESP32 1.14" ST7789 Colour LCD
        #if (SCR_ORIENT == 0)           // portrait
