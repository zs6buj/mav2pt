...
v1.0.9  2019-01-05 Fix format bug in #24 debug Debug.print(ap_eph)and Debug.print(ap_epv). Int not float.                                     
*/
...
void loop()  {
  ...
  if(mavGood && ((millis() - sp_millis) > 0)) {   // fine tuning
  ...
  if (!MavRingBuff.isEmpty()) {
    ...
    switch(msg.msgid) {
        ...
        case MAVLINK_MSG_ID_HEARTBEAT:    // #0   http://mavlink.org/messages/common
          ...
          // Ignore heartbeats from GCS (6) or Ant Trackers(5) or ADSB
          ap_type = ap_type_tmp;
