...
//=================================================================================================
    ...
    if (!EEPROM.begin(EEPROM_SIZE))  {
      ...
      log.println("NVS initialised successfully");
      LogScreenPrintln("NVS good"); 
     }
  #endif       
  ...
  #if ((defined ESP32) || (defined ESP8266)) && (defined Debug_SRAM)
    log.printf("==============>Free Heap after NVS setup = %d\n", ESP.getFreeHeap());
  #endif
    ...
    if (resetEepromPress) {
      if (eepromStart == 0) {
        ...
        if (eepromElapsed >= eepromTrigger) {
          log.printf("Pin %u:%u  interrupt ** ALL SETTINGS IN NVS SET TO COMPILE_TIME DEFAULTS! **\n", resetEepromPin, pinval ); 
          LogScreenPrintln("*NVS RESET*");  
          RawSettingsToStruct();                                         
