...
//=================================================================================================
  ...
  private:  
    ...
    if (set.frport == f_port) {
      ...
      Log.printf("F.Port at %d baud is NOT INVERTED", frBaud);     
    } else 
      if (set.frport == s_port) {
        frBaud = 57600;
        #if defined Uninverted_FrSky_Receiver_Pad // note S.Port is normally idle low (inverted)
          frInvert = false;  
          Log.printf("S.Port at %d baud is NOT INVERTED", frBaud); 
        #else
          frInvert = true;  
          Log.printf("S.Port at %d baud is INVERTED", frBaud);  
        #endif  
      }
      ...
      frTx = fr_txPin;
          ...
          if ((set.trmode == air) || (set.trmode == relay) || (set.mavlite_support)) {
            Log.printf(" on pins rx = %d and tx = %d\n", frRx, frTx);  
            Log.println("A 2-wire to 1-wire converter or diode is required");
          } else          
          if (set.trmode == ground)  {                        
            Log.printf(" and is 1-wire simplex on tx pin = %d\n", frTx);
          } 
    ...
    void FrSkyPort::SPort_Read_And_Write(uint8_t *buf) {
      ...
      while (frSerial.available())   {  // Receive sensor IDs from X receiver
        ...
        if (byt == 0x7E) {
          //Log.printf("Found 0x7E, next 0x%X and 0x%X\n", *(buf+1), *(buf+2));
          if ((*(buf+1) == 0x0D) && (*(buf+2) == 0x30)) { // MavLite downlink (to FC) match found
...
*/
    ...
    void FrSkyPort::crcStepOut(uint8_t b) {
       ...
       if (printcrcout) Log.printf("AddIn %3d %2X\tcrcout_now=%3d %2X\n", b, b, crcout, crcout);
    }   
    //=======================================================================  
   ...
   // Log.printf("length=%d\t", length);
    ...
    void FrSkyPort::PrintFPortBuffer(uint16_t lth) {
      for (int i = 0 ; i < lth+3 ; i++) {   // start_byte[0] + lth[1] + [ payload (from type[2]) ] + CRC
        if (fpbuf[i] <= 0xf) Log.print("0");
        Log.print(fpbuf[i],HEX);
        Log.write(" ");
      }
      Log.println();
    }  
    ...
    void FrSkyPort::ReportOnlineStatus() {
       ...
       if (FrSkyPort::frGood != FrSkyPort::frPrev) {  // report on change of status
         ...
         if (frGood) {
           Log.println("FrSport read good!");
           LogScreenPrintln("FrSport read ok");         
         } else {
          Log.println("FrSport read timeout!");
          LogScreenPrintln("FrSport timeout");         
         }
       }
    ...
    bool FrSkyPort::DecodeAndDownlinkMavLite() {
      ...
      if (FrSkyPort::UnchunkMavLitePayload()) {  // when fully expanded
        ...
        switch (mt_msg_id) {                // Decode uplink mavlite messages according to msg-id
          ...
          case 20:                          //  #20 or 0x14   Mavlink_Param_Request_Read - 
            {
              for (int i = 0; i <16; i++) {
                ...
                Log.printf("MavLite #20 Param_Request_Read :%s:\n", ap22_param_id);  
              #endif  
          ...
          case 23:                          // #23 or 0x17  PARAM_SET - Uplink
            {
              ...
              #if defined Debug_Mavlite
                 Log.printf("MavLite #23 Param_Set. Parameter-id= %s  Value = %.5f\n", ap23_param_id, ap23_param_value);  
              #endif  
              return true; 
            }            
          ...
          case 76:                          // #76 or 0x4c  COMMAND_LONG
            {
              ...
              #if defined Debug_Mavlite
               Log.printf("MavLite #76 Command_Long: Command=%d  confirm=%d param_count=%d ", ap76_command, ap76_confirm, par_cnt);  
          ...
          default:
            #if defined Debug_Mavlite
              Log.printf("Mavlite Uplink: Unknown Message ID #%d ignored\n", mt_msg_id);
              PrintMavLiteUplink();
            #endif
            return false;   
        } // end of switch
    ...
    bool FrSkyPort::UnchunkMavLitePayload() {              
      ...
      //    Log.printf("XX i=%d  mt_idx=%d frbuf[i]=%x \n",i, mt_idx, frbuf[i]);
     ...
     //     Log.printf("YY i=%d  mt_idx=%d frbuf[i]=%x \n",i, mt_idx, frbuf[i]);
          mt_payload[mt_idx] = frbuf[i];
          if (mt_idx >= mt_paylth) {
    ...
    //=================================================================== 
    void FrSkyPort::PushMessage(uint16_t msg_id, uint8_t sub_id) {    // Uplink to GCS
      switch(msg_id) {
    ...
    //=================================================================== 
    void FrSkyPort::PushToEmptyRow(uint16_t msg_id, uint8_t sub_id) {  // Uplink to GCS
      sb_row = 0;
      while (sb[sb_row].inuse) {   // find empty F.Port row 
        sb_row++; 
        if (sb_row >= sb_rows-1) {
          if ( (sb_buf_full_gear == 0) || (sb_buf_full_gear%4000 == 0)) {
            Log.println("FrPort scheduler buffer full. Check FrPort Uplink to GCS");  // Report every so often
          }
          sb_buf_full_gear++;
          return;     
        }
    ...
    void FrSkyPort::Push_Param_Val_016(uint16_t msg_id) {   //  0x16 MavLite PARAM_VALUE ( #22 ) Uplink to GCS
    #if defined MavLite_Support  
      ...
      Log.printf("Mavlite #22 uplink to gcs lth=%d\n", mt_totlth);    
      
      #if (defined Debug_Mavlite)
          Log.printf("PushMavLiteParamValue16 msg_id=0x%X lth=%d param_value=%.3f param_id=%s \n"
    ...
    //===================================================================     
    #if defined MavLite_Support  // Uplink to GCS
    ...
    void FrSkyPort::ChunkMavLitePayload(uint8_t msg_id, uint8_t tlth) {
      ...
      //       Log.printf("mt_idx=%d  mt_payload.raw[mt_idx]=0x%X\n", mt_idx, mt_payload.raw[mt_idx] );
        ...
        FrSkyPort::PushToEmptyRow(msg_id, 0);  
          ...
          Log.printf("MavLite Uplink to GCS chunk 0x%x: ", msg_id);
          Log.printf(" seq=%d  ", mtf_payload.seq);
          pb_rx = false;
    ...
    void FrSkyPort::Push_Bat1_5003(uint16_t msg_id) {   //  Into F.Port table from #1 SYS_status only
      ...
      //pt_bat1_mAh = Total_mAh1();  // If record type #147 is not sent and good
        ...
        Log.print(" pt_bat1_volts="); Log.print(pt_bat1_volts);
        Log.print(" pt_bat1_amps="); Log.print(pt_bat1_amps);
    ...
    void FrSkyPort::Push_Rssi_F101(uint16_t msg_id) {          // msg_id 0xF101 RSSI tell LUA script in Taranis we are connected
      ...
      bit32Pack(pt_rssi ,0, 32);
        ...
        PrintFrPeriod(0);    
        Log.print("Passthru out Rssi 0x5F101: ");   
    ...
    void FrSkyPort::PrintPayload(msg_class_t msg_class)  {
      ...
      uint8_t sz = 0;
      if (msg_class == passthru) {
        Log.print(" passthru payload ");
        bytes = (uint8_t*)&pt_payload;         // cast to bytes
        sz = 4;   
      } else
      if (msg_class == mavlite) {
        Log.print(" MavLite payload ");   
        bytes = (uint8_t*)&mtf_payload;           // cast to bytes
        sz = 6;
      } 
