...
*/
    ...
    void FrSkyPort::Send_Frsky_Frame(uint16_t idx, uint8_t _prime) {
      ...
      if (sp_msg_id == 0xF101) {                   
        ...
        #endif   
        if (set.trmode != relay) { 
          if ( (frport_type == s_port) || (frport_type == f_port1) )  {
      ...
      if (set.trmode == ground) {             // In ground mode we are master and send a diy poll
        ...
        if (frport_type == f_port1) {   // FP1 and 2 in ground mode are conjecture right now until I can get a upgraded transmitter to test with
...
if (ap24_sat_visible > 15) {                // @rotorman 2021/01/18
  if (ap24_sat_visible == 255)
    ...
    void FrSkyPort::Push_Bat1_5003(uint16_t msg_id) {   //  Into F.Port table from #1 SYS_status only
      pt_payload = 0;
      pt_bat1_volts = (float)ap_voltage_battery1 / 100;         // Were mV, now dV  - V * 10
      pt_bat1_amps = (float)ap_current_battery1  / 10;          // Were cA, now dA  - A * 10   
      ...
      pt_bat1_amps = prep_number(roundf(pt_bat1_amps * 0.1F),2,1);    // NOTE, (0.1F) rounds to whole Amps for dA   
