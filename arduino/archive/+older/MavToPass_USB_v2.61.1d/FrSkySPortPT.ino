...
//====================================   D O W N L I N K  =========================================   
 void SPortFrameAndSend(uint16_t idx) {
  ...
  if (msg_class == mavlite) {
    ...
    mt_payload[mt_row] = {};  // clear payload row struct
...
//=================================================================================================  
//                    M A V L I T E   P A C K   M E S S A G E  P A Y L O A D S
//================================================================================================= 
...
*/
  ...
  while (mtmsg_idx <= mt22_tot) {    
    ...
    if (mt_chunk == 0) {
      ...
      // first chunk 
      mt_Payload.seq = mt_chunk;      AddInCrc(mt_Payload.lth);
      mt_Payload.lth = mliteMsg.lth;  AddInCrc(mt_Payload.lth);     // idx = 1          
      mt_Payload.msg_id = msg_id;     AddInCrc(mt_Payload.msg_id);  // idx = 0

      //______________________________________________________
      for (int i = 3 ; i < 6 ; i++) {  
        mt_Payload.raw[i] = mliteMsg.raw[mtmsg_idx];  
        AddInCrc(mt_Payload.raw[i]); 
 /*
...
*/
      ...
      mt_Payload.seq = mt_chunk;  AddInCrc(mt_Payload.seq); 
      //_______________________________________________________
      for (int i = 1 ; i < 6 ; i++) { 
          
        if (mtmsg_idx >= mt22_tot) {  // break out of the for loop
         mt_Payload.raw[i] = 0xFF-CRC;     // append the CRC
