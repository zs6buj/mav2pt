...
v1.0.19 2019-07-17 PX4 flight stack only - fixed longitude typo    if (ap_lat24<0) should be if (ap_lon24<0)                                                                
*/
...
int32_t    ap_lat24 = 0;           // 7 assumed decimal places - use ap_lat and ap_lon for interoperability between APM and PX4
...
int16_t   ap_voltage_battery2 = 0;    // 1000 = 1V
...
void setup()  {
  ...
  Debug.print("Running sketch name: ");
...
#if defined LRS_RSSI 
    Debug.println("LRS_RSSI variant"); 
#endif
  ...
  if (!MavRingBuff.isEmpty()) {
    ...
    switch(msg.msgid) {
        ...
        case MAVLINK_MSG_ID_SYS_STATUS:   // #1
          if (!mavGood) break;
          int16_t raw_voltage;
          raw_voltage = mavlink_msg_sys_status_get_voltage_battery(&msg);  // 1000 = 1V  i.e mV
          ap_voltage_battery1 = Get_Volt_Average1(raw_voltage);  
          ...
          if(ap_voltage_battery1> 21000) ap_ccell_count1= 6;
            ...
            Debug.print(" Raw volts =");
            Debug.print((float)raw_voltage/ 1000, 3);   // now V     
        ...
        case MAVLINK_MSG_ID_GPS_RAW_INT:          // #24 - Used by PX4
          ...
          if(ap_fixtype > 2)  {
            ap_lat24 = mavlink_msg_gps_raw_int_get_lat(&msg);
            ap_lon24 = mavlink_msg_gps_raw_int_get_lon(&msg);
        ...
        case MAVLINK_MSG_ID_GLOBAL_POSITION_INT:     // #33 used by APM
          ...
          ap_lat33 = mavlink_msg_global_position_int_get_lat(&msg);             // Latitude, expressed as degrees * 1E7
          ap_lon33 = mavlink_msg_global_position_int_get_lon(&msg);             // Pitch angle (rad, -pi..+pi)
        ...
        case MAVLINK_MSG_ID_BATTERY_STATUS:      // #147   http://mavlink.org/messages/common
          ...
          if (ap_current_consumed < 0) { //  fix for px4 where bat mA drift negative sometimes on start up
            ap_current_consumed = 0;
           }
