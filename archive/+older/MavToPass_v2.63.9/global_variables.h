...
// Message #20 PARAM_REQUEST_READ     // Uplink - request to read the onboard parameter with the param_id string id
...
// Message #51 Mission_Request_Int    Uplink - Request info on mission seq #
...
//Command_Long ( #76 )          // Downlink to FC-  https://mavlink.io/en/services/command.html
// target sys and comp defined up top 
...
//Command_Ack ( #77 )           // Uplink from FC https://mavlink.io/en/services/command.html
...
uint32_t pt_bat1_capacity;
uint32_t pt_bat2_capacity;
...
//=================================================================================================
    ...
    typedef enum wfmode_set { ap = 1 , sta = 2, sta_ap = 3, ap_sta = 4 } wfmode_t;   
    typedef enum wf_proto_set { tcp = 1 , udp = 2, } wf_proto_t; 
    typedef enum btmode_set { master = 1, slave = 2 } btmode_t;
    typedef enum frport_type_set { f_none = 0, s_port = 1 , f_port = 2 } frport_t;       
    typedef struct  {
      ...
      frport_t      frport;                // 162 in EEPROM
      
      // below is not saved in EEPROM
      uint8_t       rssi_override;     // this variable is not saved in eeprom
      bool          mavlite_support;   // this flag is not saved in eeprom
