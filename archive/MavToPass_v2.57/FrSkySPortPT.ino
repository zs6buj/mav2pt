...
void FrSkySPort_Init(void)  {
  ...
  frTx = Fr_txPin;
    ...
    Debug.printf("ESP S.Port txpin inverted to half-duplex single wire on pin %d \n", frTx); 
  #else
    frInvert = false;
    Debug.println("ESP S.Port rxpin=%d and txpin=%d need an inverter/single-wire converter", frRx, frTx);
  #endif
...
//=================================================================================================  
#if defined ESP32 || defined ESP8266
  void frSerialSafeWrite(SoftwareSerial* sws, byte b) {
    sws->enableTx(true);
    frSerial.write(b); 
   sws->enableTx(false);
  }
