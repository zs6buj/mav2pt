...
// ************************ Setup Bluetooth ***************************  
  #if (FC_Mavlink_IO == 1) || (GCS_Mavlink_IO == 1) // Bluetooth 
    SerialBT.begin("ESP32", BT_Master_Mode);            // true = Bluetooth as master
...
void main_loop() {
  ...
  if (((rssiGood) || ((RSSI_Source == 3) && mavGood)) && (millis() - rssi_millis > 800)) {
  ...
  void SetupWiFi() { 
    ...
    bool sta_no_connect = false;
      ...
      while (WiFi.status() != WL_CONNECTED){
        retry++;
        if (retry > 10) {
          sta_no_connect = true;   // Rather go establish an AP instead
          break;
        }
        delay(500);
        Serial.print(".");
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        Debug.print("  port: ");
        #if   (WiFi_Protocol == 1)   // TCP
          Debug.println(tcp_localPort);
        #elif (WiFi_Protocol == 2)   // UDP
          Debug.println(udp_localPort);
        #endif  
        wifi_rssi = WiFi.RSSI();
    ...
    #if (WiFi_Mode == 1) || (sta_no_connect)  // AP
