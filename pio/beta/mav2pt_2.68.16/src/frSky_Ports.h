...
//=================================================================================================
  ...
  uint16_t  prepNumber(int32_t, uint8_t, uint8_t);
  ...
  if (ap24_sat_visible == 255)
    ...
    pt_numsats = 15; // limit to 15 due to only 4 bits available
     ...
     pt_numsats = ap24_sat_visible;
      ...
      pt_amsl = prepNumber(pt_amsl,2,2);                        
      pt_hdop = prepNumber(pt_hdop,2,1);
    ...
    void FrSkyPort::Push_Bat1_5003(uint16_t msg_id) {   //  Into F.Port table from #1 SYS_status only
      ...
      uint16_t w_bat1_amps = prepNumber(roundf(pt_bat1_amps),2,1);
    ...
    void FrSkyPort::Push_Home_5004(uint16_t msg_id) {
      ...
      pt_home_dist = prepNumber(roundf(pt_home_dist), 3, 2);
      bit32Pack(pt_home_dist ,0, 12);
      pt_home_alt = prepNumber(roundf(pt_home_alt), 3, 2);
    ...
    void FrSkyPort::Push_VelYaw_5005(uint16_t msg_id) {
      ...
      if (pt_vy<0)
        ...
        bit32Pack(0, 8, 1);
      pt_vy = prepNumber(roundf(pt_vy), 2, 1);  // Vertical velocity
      bit32Pack(pt_vy, 0, 8);   

      pt_vx = prepNumber(roundf(pt_vx), 2, 1);  // Horizontal velocity
      bit32Pack(pt_vx, 9, 8);    
    ...
    void FrSkyPort::Push_Atti_5006(uint16_t msg_id) {
      ...
      bit32Pack(prepNumber(pt_range,3,1), 21, 11);
    ...
    void FrSkyPort::Push_Bat2_5008(uint16_t msg_id) {
      ...
      pt_bat2_amps = prepNumber(roundf(pt_bat2_amps * 0.1F),2,1);          
    ...
    7 - up - left 
      ...
      pt_ms_dist = prepNumber(roundf(pt_ms_dist), 3, 2);       //  number, digits, power
      bit32Pack(pt_ms_dist, 10, 12);    
      pt_ms_xtrack = prepNumber(roundf(pt_ms_xtrack), 1, 1);  
    ...
    void FrSkyPort::Push_Terrain_500B(uint16_t msg_id) {
      pt_payload = 0;
      pt_height_above_terrain = prepNumber(roundf(ap136_current_height*10), 3, 2);
    ...
    void FrSkyPort::Push_VFR_Hud_50F2(uint16_t msg_id) {
      ...
      pt_air_spd = prepNumber(roundf(pt_air_spd), 2, 1);  
      bit32Pack(pt_air_spd, 0, 8);    

      bit32Pack(pt_throt, 8, 7);

      pt_bar_alt =  prepNumber(roundf(pt_bar_alt), 3, 2);
   ...
   void FrSkyPort::PrintBuffer(uint8_t *buf, uint8_t length) {
    ...
    uint32_t FrSkyPort::getBaud(int8_t s_pin, pol_t pl) {
