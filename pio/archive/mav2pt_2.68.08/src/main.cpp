...
// #include "lilygo_t_display_s3_pins_arduino.h" //
...
*/
  ...
  if (resetEepromPin != -1)
  {                                 // Pin to reset eeprom to default settings on the fly
...
#error You must define at least one Battery_mAh_Source. Please correct.
  ...
  else if (set.fr_io == fr_udp)
  { // 2
    log.println("FrSky UDP");
    LogScreenPrintln("Frs UDP");
  }
  else if (set.fr_io == fr_ser_udp)
  { // 3
  ...
  else if (set.fr_io == fr_ser_udp_sd)
  { // 7
  ...
  */
  ...
  //=================================================================================================
  if ((set.fc_io == fc_ser) || (set.gs_io == gs_ser))
  { //  Serial
...
void loop()
{
  ...
  if (gshbGood && (millis() - gshb_millis) > 10000)
  { // if no heartbeat from GCS in n seconds then assume GCS not connected
    gshbGood = false;
  ...
  if (mavGood && (millis() - hb_millis) > 15000)
  { // if no heartbeat from APM in 15s then assume FC mav not connected
    mavGood = false;
  ...
  if ((set.trmode == ground) || (set.trmode == relay))
  { // In Air_Mode the FrSky receiver provides rssi
...
bool readFCtoRingBuffer()
{
...
#ifdef Debug_FC_Down
      log.print("Read from FC Bluetooth to Ringbuffer: msgReceived=");
      log.println(msgReceived);
      printMavBuffer(&F2Rmsg);
#endif
    }
    return true;
  ...
  { //  WiFi
    ...
    if ((set.mav_wfproto == tcp) && (outbound_clientGood))
    { // TCP  from FC side
...
#ifdef Debug_FC_Down
        log.print("Read from FC WiFi TCP to Ringbuffer: msgReceived=");
        log.println(msgReceived);
        printMavBuffer(&F2Rmsg);
#endif
      }
    ...
    { // UDP from FC
      ...
      if ((set.wfmode == ap_sta) || (set.wfmode == sta))
      {                                     // if AP_STA or STA mode
      ...
      if (msgReceived)
      {
        ...
        log.print("Read WiFi UDP from FC to G2Fmsg: msgReceived=");
        log.println(msgReceived);
#endif
        ...
        log.print("Read from FC WiFi UDP to Ringbuffer: msgReceived=");
        log.println(msgReceived);
        printMavBuffer(&F2Rmsg);
#endif
      }
...
#ifdef Debug_GCS_Up
      log.print("Passed up from GCS BT to G2Fmsg: msgReceived=");
      log.println(msgReceived);
      if (msgReceived)
        printMavBuffer(&G2Fmsg);
#endif
...
#if (defined wifiBuiltin)
  if ((set.gs_io == gs_wifi) || (set.gs_io == gs_wifi_bt))
  { //  WiFi

    if ((set.mav_wfproto == tcp) && (inbound_clientGood))
    { // TCP from GCS side
...
#ifdef Debug_readTCP
        log.print("Read WiFi TCP to G2Fmsg: msgReceived=");
        log.println(msgReceived);
        if (msgReceived)
          printMavBuffer(&G2Fmsg);
#endif
    ...
    { // UDP from GCS
      ...
      if ((set.wfmode == ap_sta) || (set.wfmode == ap))
      {                                    // if AP_STA mode
...
{
  if ((!wifiSuGood) || ((!inbound_clientGood) && (!outbound_clientGood)))
    return false;
  ...
  { // check each connected client for waiting data
    if (NULL != tcp_client[i])
    { // if null pointer go around, tcp_client[0] is reserved for inbound
      // active client
      active_client_idx = i;
...
void decodeGCStoFC()
{
  if ((set.gs_io == gs_ser) || (set.gs_io == gs_bt) || (set.gs_io == gs_wifi) || (set.gs_io == gs_wifi_bt))
  { // if any GCS I/O requested
    ...
    case MAVLINK_MSG_ID_HEARTBEAT: // #0
      ...
      log.print("apo_sysid=");
      log.print(ap_sysid);
      log.print("  apo_compid=");
      log.print(ap_compid);
      log.print("  apo_type=");
      log.print(ap_type);
      log.print("  apo_autopilot=");
      log.print(ap_autopilot);
      log.print("  apo_base_mode=");
      log.print(ap_base_mode);
      log.print(" apo_custom_mode=");
      log.print(ap_custom_mode);
      log.print("  apo_system_status=");
      log.print(ap_system_status);
      log.print("  apo_mavlink_version=");
      log.print(ap_mavlink_version);
      log.println();
#endif
      break;
    ...
    case MAVLINK_MSG_ID_PARAM_REQUEST_READ: // #20  from GCS
      ...
      log.print("apo_targsys=");
      log.print(apo_targsys);
      log.print("  gcs_req_param_id=");
      log.print(gcs_req_param_id);
      log.print("  gcs_req_param_index=");
      log.print(gcs_req_param_index);
      log.println();
    ...
    case MAVLINK_MSG_ID_MISSION_REQUEST_INT: // #51 To FC from GCS
      ...
      log.print("  apo_seq=");
      log.print(apo_seq);
      log.print("  apo_mission_type=");
      log.print(apo_mission_type); // Mav2
      log.println();
#endif
      break;
    ...
    case MAVLINK_MSG_ID_COMMAND_LONG: // #76 To FC from GCS
      ...
      log.print("  apo_targcomp=");
      log.print(apo_targcomp);
      log.print("  apo76_command=");
      log.print(apo76_command);
      log.print("  apo76_confirmation=");
      log.print(apo76_confirmation);
      for (int i = 0; i < 7; i++)
      {
...
void sendToFC(uint32_t msg_id)
{
  ...
  { // Serial to FC
    ...
    if (msg_id)
    { //  dont print heartbeat - too much info
...
#ifdef Debug_FC_Up
    log.print("Sent to FC Bluetooth from G2Fmsg: msgSent=");
    log.println(msgSent);
    if (msgSent)
      printMavBuffer(&G2Fmsg);
#endif
  ...
  { // WiFi to FC
    if (wifiSuGood)
    {
      ...
      {                                  // TCP
        ...
        msgSent = msgSent;               // silence irritating compiler warning
#ifdef Debug_GCS_Up
        log.print("Sent to FC WiFi TCP from G2Fmsg: msgSent=");
        log.println(msgSent);
        printMavBuffer(&G2Fmsg);
#endif
      }
      ...
      { // UDP
        ...
        log.print("Sent to FC WiFi UDP from G2Fmsg: msgSent=");
        log.println(msgSent);
        if (msgSent)
          printMavBuffer(&G2Fmsg);
#endif
...
void Send_From_RingBuf_To_GCS()
{ // Down to GCS (or other) from Ring Buffer
  ...
  if ((set.gs_io == gs_ser) || (set.gs_io == gs_bt) || (set.gs_io == gs_wifi) || (set.gs_io == gs_wifi_bt) || (set.gs_sd == gs_on))
  {
    ...
    { // Bluetooth
      ...
      msgSent = msgSent; // silence irritating compiler warning
#ifdef Debug_GCS_Down
      log.print("Sent to GCS Bluetooth from R2Gmsg: msgSent=");
      log.println(msgSent);
      if (msgSent)
        printMavBuffer(&G2Fmsg);
#endif
    ...
    { //  WiFi
      if (wifiSuGood)
      {
        ...
        { // TCP
          for (int i = 1; i < max_clients; ++i)
          { // send to each active client. Not to inbound client [0] (FC side)
            if (NULL != tcp_client[i])
            { // if active client
              active_client_idx = i;
...
//  printMavBuffer(&R2Gmsg);
...
#else
          for (int i = 1; i < max_clients; i++)
          { // send to each individual remote udp ip. Not to FC side client ip [0]
            if (udpremoteip[i][0] != 0)
            {
              ...
              log.print(" Sent to GCS by WiFi UDP remote IP=");
              log.println(UDP_remoteIP);
              printMavBuffer(&R2Gmsg);
#endif
            }
          }
    ...
    { //  GCS downlink telem to SD Card
      ...
      { //  if open for write
        ...
        if (file.write(GCSbuf, len + 18))
        { // 8 bytes plus some head room
        }
        else
        {
...
void CheckMavTimeouts()
{
  ...
  if (ap_fence_enabled && (millis() - ap_fence_last_update) > 2000)
  {
    ...
    log.print("  fence_enabled= ");
    log.print(ap_fence_enabled ? "true" : "false");
    log.print("  breach_status= ");
    log.println(ap_breach_status);
#endif
  }
}
//=================================================================================================
void DecodeOneMavFrame()
{
  ...
  if (mavGood && (ap_sysid != R2Gmsg.sysid || ap_compid != R2Gmsg.compid))
    return; // If messages are not from our autopilot
  ...
  case MAVLINK_MSG_ID_HEARTBEAT:                                     // #0   https://mavlink.io/en/messages/common.html
    ...
    if (ap_type_tmp == 5 || ap_type_tmp == 6 || ap_type_tmp == 18 || ap_type_tmp >= 26 || ap_type_tmp == 27 || ap_autopilot_tmp == 8)
      break;
    ...
    if (motArmed && (ap24_fixtype >= 6) && (!homGood))
    {             // AND 3D Fix
    ...
    if (!mavGood)
    {
      ...
      if (hb_count >= 3)
      { // If  3 heartbeats from MavLink then we are connected
        mavGood = true;
    ...
    if (px4_flight_stack)
    {
      log.print(" px4_main_mode=");
      log.print(px4_main_mode);
      log.print(" px4_sub_mode=");
      log.print(px4_sub_mode);
      log.print(" ");
      log.print(PX4FlightModeName(px4_main_mode, px4_sub_mode));
    }
  ...
  case MAVLINK_MSG_ID_SYS_STATUS: // #1
    ...
    log.print("  mAh=");
    log.print(bat1.mAh, 6);
    log.print("  Total mAh=");
    log.print(bat1.tot_mAh, 3); // Consumed so far, calculated in Average module
  ...
  case MAVLINK_MSG_ID_SYSTEM_TIME: // #2
    ...
    log.print(" Unix secs=");
    log.print((float)(ap_time_unix_usec / 1E6), 6);
    log.print("  Boot secs=");
    log.println((float)(ap_time_boot_ms / 1E3), 0);
#endif
    break;
  ...
  case MAVLINK_MSG_ID_PARAM_VALUE: // #22
    ...
    { // if matched
      ...
      log.print(ap22_param_id);
      log.print(" ap22_param_value=");
      log.println(ap22_param_value, 3);
#endif
    ...
    switch (ap22_param_index)
    {         // if #define Battery_mAh_Source !=1 these will never arrive
  ...
  case MAVLINK_MSG_ID_GPS_RAW_INT: // #24
    ...
    log.print("ap24_fixtype=");
    log.print(ap24_fixtype);
    if (ap24_fixtype == 0)
      log.print(" No GPS");
    else if (ap24_fixtype == 1)
      log.print(" No Fix");
    else if (ap24_fixtype == 2)
      log.print(" 2D Fix");
    else if (ap24_fixtype == 3)
      log.print(" 3D Fix");
    else if (ap24_fixtype == 4)
      log.print(" DGPS/SBAS aided");
    else if (ap24_fixtype == 5)
      log.print(" RTK Float");
    else if (ap24_fixtype == 6)
      log.print(" RTK Fixed");
    else if (ap24_fixtype == 7)
      log.print(" Static fixed");
    else if (ap24_fixtype == 8)
      log.print(" PPP");
    else
      log.print(" Unknown");

    log.print("  sats visible=");
    log.print(ap24_sat_visible);
    log.print("  latitude=");
    log.print((float)(ap24_lat) / 1E7, 7);
    log.print("  longitude=");
    log.print((float)(ap24_lon) / 1E7, 7);
    log.print("  gps alt amsl=");
    log.print((float)(ap24_amsl) / 1E3, 1);
    log.print("  eph (hdop)=");
    log.print((float)ap24_eph); // HDOP
    log.print("  epv (vdop)=");
    log.print((float)ap24_epv);
    log.print("  vel=");
    log.print((float)ap24_vel / 100, 3); // GPS ground speed (m/s)
    log.print("  cog=");
    log.print((float)ap24_cog / 100, 1); // Course over ground in degrees
    //  mav2
    log.print("  alt_ellipsoid)=");
    log.print(ap24_alt_ellipsoid / 1000, 2); // alt_ellipsoid in mm
    log.print("  h_acc=");
    log.print(ap24_h_acc); // Position uncertainty in mm. Positive for up.
    log.print("  v_acc=");
    log.print(ap24_v_acc); // Altitude uncertainty in mm. Positive for up.
    log.print("  ap24_vel_acc=");
    log.print(ap24_vel_acc); // Speed uncertainty. Positive for up.
    log.print("  ap24_hdg_acc=");
    log.print(ap24_hdg_acc); // degE5   Heading / track uncertainty
    log.println();
#endif
  ...
  case MAVLINK_MSG_ID_SCALED_IMU: // #26
    ...
    log.print("xacc=");
    log.print((float)ap26_xacc / 1000, 3);
    log.print("  yacc=");
    log.print((float)ap26_yacc / 1000, 3);
    log.print("  zacc=");
    log.print((float)ap26_zacc / 1000, 3);
    log.print("  xgyro=");
    log.print((float)ap26_xgyro / 1000, 3);
    log.print("  ygyro=");
    log.print((float)ap26_ygyro / 1000, 3);
    log.print("  zgyro=");
    log.print((float)ap26_zgyro / 1000, 3);
    log.print("  xmag=");
    log.print((float)ap26_xmag / 1000, 3);
    log.print("  ymag=");
    log.print((float)ap26_ymag / 1000, 3);
    log.print("  zmag=");
    log.print((float)ap26_zmag / 1000, 3);
    log.print("  temp=");
    log.println((float)ap26_temp / 100, 2); // cdegC
#endif
  ...
  case MAVLINK_MSG_ID_RAW_IMU: // #27
    ...
    log.print("accX=");
    log.print((float)ap27_xacc / 1000);
    log.print("  accY=");
    log.print((float)ap27_yacc / 1000);
    log.print("  accZ=");
    log.println((float)ap27_zacc / 1000);
    log.print("  xgyro=");
    log.print((float)ap27_xgyro / 1000, 3);
    log.print("  ygyro=");
    log.print((float)ap27_ygyro / 1000, 3);
    log.print("  zgyro=");
    log.print((float)ap27_zgyro / 1000, 3);
    log.print("  xmag=");
    log.print((float)ap27_xmag / 1000, 3);
    log.print("  ymag=");
    log.print((float)ap27_ymag / 1000, 3);
    log.print("  zmag=");
    log.print((float)ap27_zmag / 1000, 3);
    log.print("  id=");
    log.print((float)ap27_id);
    log.print("  temp=");
    log.println((float)ap27_temp / 100, 2); // cdegC
#endif
#endif
    break;
  ...
  case MAVLINK_MSG_ID_SCALED_PRESSURE: // #29
    ...
    log.print("  press_abs=");
    log.print(ap_press_abs, 1);
    log.print("hPa  press_diff=");
    log.print(ap_press_diff, 3);
    log.print("hPa  temperature=");
    log.print((float)(ap_temperature) / 100, 1);
    log.println("C");
#endif
#endif
    break;
  ...
  case MAVLINK_MSG_ID_GLOBAL_POSITION_INT: // #33
    ...
    log.print(" ap_lat=");
    log.print((float)ap33_lat / 1E7, 6);
    log.print(" ap_lon=");
    log.print((float)ap33_lon / 1E7, 6);
    log.print(" ap_amsl=");
    log.print((float)ap33_amsl / 1E3, 0);
    log.print(" ap33_alt_ag=");
    log.print((float)ap33_alt_ag / 1E3, 1);
    log.print(" ap33_vx=");
    log.print((float)ap33_vx / 100, 2);
    log.print(" ap33_vy=");
    log.print((float)ap33_vy / 100, 2);
    log.print(" ap33_vz=");
    log.print((float)ap33_vz / 100, 2);
    log.print(" ap33_gps_hdg=");
    log.println((float)ap33_gps_hdg / 100, 1);
#endif
  ...
  case MAVLINK_MSG_ID_RC_CHANNELS_RAW: // #35
    ...
    if ((!rssi65) && (!rssi109))
    { // If no #65 and no #109 received, then use #35
      rssiGood = true;
    ...
    log.print("  ap_rssi35=");
    log.print(ap_rssi35); // 0xff -> 100%
    log.print("  rssiGood=");
    log.println(rssiGood);
#endif
    break;
  ...
  case MAVLINK_MSG_ID_MISSION_ITEM: // #39
    ...
    log.print("ap_ms_seq=");
    log.print(ap_ms_seq);
    log.print(" ap_ms_frame=");
    log.print(ap_ms_frame);
    log.print(" ap_ms_command=");
    log.print(ap_ms_command);
    log.print(" ap_ms_current=");
    log.print(ap_ms_current);
    log.print(" ap_ms_autocontinue=");
    log.print(ap_ms_autocontinue);
    log.print(" ap_ms_param1=");
    log.print(ap_ms_param1, 7);
    log.print(" ap_ms_param2=");
    log.print(ap_ms_param2, 7);
    log.print(" ap_ms_param3=");
    log.print(ap_ms_param3, 7);
    log.print(" ap_ms_param4=");
    log.print(ap_ms_param4, 7);
    log.print(" ap_ms_x=");
    log.print(ap_ms_x, 7);
    log.print(" ap_ms_y=");
    log.print(ap_ms_y, 7);
  ...
  case MAVLINK_MSG_ID_MISSION_CURRENT: // #42 should come down regularly as part of EXTENDED_status group
    ...
    if (ap_ms_seq)
    {
      ...
      log.print("ap_mission_current=");
      log.println(ap_ms_seq);
    }
#endif

    if (ap_ms_seq > 0)
      ap_ms_current_flag = true; //  Ok to send passthru frames
  ...
  case MAVLINK_MSG_ID_MISSION_COUNT: // #44   received back after #43 Mission_Request_List sent
    ...
    log.print("ap_mission_count=");
    log.println(ap_mission_count);
#endif
  ...
  case MAVLINK_MSG_ID_NAV_CONTROLLER_OUTPUT: // #62
    ...
    log.print("ap_nav_roll=");
    log.print(ap_nav_roll, 3);
    log.print(" ap_nav_pitch=");
    log.print(ap_nav_pitch, 3);
    log.print(" ap_nav_bearing=");
    log.print(ap_nav_bearing);
    log.print(" ap_target_bearing=");
    log.print(ap_target_bearing);
    log.print(" ap_wp_dist=");
    log.print(ap_wp_dist);
    log.print(" ap_alt_error=");
    log.print(ap_alt_error, 2);
    log.print(" ap_aspd_error=");
    log.print(ap_aspd_error, 2);
    log.print(" ap_xtrack_error=");
    log.print(ap_xtrack_error, 2);
    log.println();
#endif
#if (defined frBuiltin)
#if defined PlusVersion
  ...
  case MAVLINK_MSG_ID_RC_CHANNELS: // #65
    ...
    if (!rssi109)
    { // If no #109 received, then use #65
      rssiGood = true;
#if defined Rssi_In_Percent
  ...
  case MAVLINK_MSG_ID_MISSION_ITEM_INT: // #73   received back after #51 Mission_Request_Int sent
    ...
    log.print("target_system =");
    log.print(ap_targsys);
    log.print("  target_component =");
    log.print(ap_targcomp);
    log.print(" _seq =");
    log.print(ap73_seq);
    log.print("  frame =");
    log.print(ap73_frame);
    log.print("  command =");
    log.print(ap73_command);
    log.print("  current =");
    log.print(ap73_current);
    log.print("  autocontinue =");
    log.print(ap73_autocontinue);
    log.print("  param1 =");
    log.print(ap73_param1, 2);
    log.print("  param2 =");
    log.print(ap73_param2, 2);
    log.print("  param3 =");
    log.print(ap73_param3, 2);
    log.print("  param4 =");
    log.print(ap73_param4, 2);
    log.print("  x =");
    log.print(ap73_x);
    log.print("  y =");
    log.print(ap73_y);
    log.print("  z =");
    log.print(ap73_z, 4);
    log.print("  mission_type =");
    log.println(ap73_mission_type);
  ...
  case MAVLINK_MSG_ID_VFR_HUD: //  #74
    ...
    log.print("Airspeed= ");
    log.print(ap74_air_spd, 2); // m/s
    log.print("  Groundspeed= ");
    log.print(ap74_grd_spd, 2); // m/s
    log.print("  Heading= ");
    log.print(ap74_hdg); // deg
    log.print("  Throttle %= ");
    log.print(ap74_throt); // %
    log.print("  Baro alt= ");
    log.print(ap74_amsl, 0); // m
    log.print("  Climb rate= ");
    log.println(ap74_climb); // m/s
#endif
  ...
  case MAVLINK_MSG_ID_COMMAND_ACK: // #77   received back after #76 Command_Long
    ...
    log.print("command=");
    log.print(ap77_command);
    log.print(" result=");
    log.print(ap77_result);
    log.print(" progres=");
    log.print(ap77_progress);
    log.print(" result_parm2=");
    log.print(ap77_result_param2);
    log.print(" target_system=");
    log.print(ap77_target_system);
    log.print(" target_component=");
    log.println(ap77_target_component);
    break;
#endif
  ...
  case MAVLINK_MSG_ID_RADIO_STATUS: // #109
    ...
    log.print("ap109_rssi=");
    log.print(ap109_rssi);
    log.print("  remrssi=");
    log.print(ap109_remrssi);
    log.print("  txbuf=");
    log.print(ap109_txbuf);
    log.print("  noise=");
    log.print(ap109_noise);
    log.print("  remnoise=");
    log.print(ap109_remnoise);
    log.print("  rxerrors=");
    log.print(ap109_rxerrors);
    log.print("  fixed=");
    log.print(ap109_fixed);
    log.print("  rssiGood=");
    log.println(rssiGood);
#endif
  ...
  case MAVLINK_MSG_ID_TERRAIN_REPORT: // #136   https://mavlink.io/en/messages/common.html
    ...
    log.print("  current_height= ");
    log.println(ap136_current_height);
#endif
  ...
  case MAVLINK_MSG_ID_BATTERY_STATUS: // #147   https://mavlink.io/en/messages/common.html
    ...
    log.print(" bat id= ");
    log.print(ap_battery_id);
    log.print(" bat current mA= ");
    log.print(ap_current_battery * 10); // now shows mA
    log.print(" ap_current_consumed mAh= ");
    log.print(ap_current_consumed);
    if (ap_battery_id == 0)
    {
  ...
  case MAVLINK_MSG_ID_BATTERY2: // #181   https://mavlink.io/en/messages/ardupilotmega.html
    ...
    log.print("  mAh=");
    log.print(bat2.mAh, 6);
    log.print("  Total mAh=");
    log.print(bat2.tot_mAh, 3);
  ...
  case MAVLINK_MSG_ID_HOME_POSITION: // #242
    ...
    log.print(" lat=");
    log.print((float)(ap242_latitude / 1E7), 7);
    log.print(" lon=");
    log.print((float)(ap242_longitude / 1E7), 7);
    log.print(" alt_msl=");
    log.print(ap242_alt_msl / 1E3, 0);
    log.print(" cur.hdg=");
  ...
  case MAVLINK_MSG_ID_STATUSTEXT: // #253
    ...
    log.print(" Severity=");
    log.print(ap_severity);
    log.print(" ");
    log.print(MavSeverity(ap_severity));
    log.print("  Text= ");
    log.print(" |");
    log.print(ap_text);
    log.println("| ");
...
//=================================================================================================
void mavParamRequestRead(int16_t param_index, char *param_id)
{                             // #20
  apo_sysid = Device_sysid;   // From config.h MP is 255, QGC default is 0
  apo_compid = Device_compid; // 158 Generic autopilot peripheral component ID. MP is 190
...
{                             //  #40
  ...
  sendToFC(40);
#if defined Mav_Debug_All || defined Mav_Debug_Missions
  log.print("Mavlink to FC #40 Request Mission:  ms_seq=");
  log.println(ms_seq);
#endif
}
#endif
...
void RequestAllWaypoints(uint16_t ms_count)
{
  ...
  { //  Mission count = next empty WP, i.e. one too high
    RequestMission(i);
  }
}
#endif
...
//=================================================================================================
void Mavlink_Request_Home_Position()
{ // #410  https://mavlink.io/en/messages/common.html#MAV_CMD
  apo76_command = 410;
  apo76_param[0] = 0;
  apo76_param[1] = 0;
  apo76_param[2] = 0;
...
void serviceWiFiRoutines()
{
  ...
  if ((set.wfmode == sta) && wifiSuGood && wifiSuDone)
  {                       // in sta mode check for disconnect from AP
  ...
  if (AP_sta_count > AP_prev_sta_count)
  {
    ...
    if ((set.fc_io == fc_wifi) && (set.mav_wfproto == tcp))
    {                           // if we expect wifi from the fc, we are a client
...
void serviceInboundTCPClients()
{
  ...
  if ((set.gs_io == gs_wifi) || (set.gs_io == gs_wifi_bt))
  { // Only when we expect incoming remote tcp_client
...
{ // interrupt for periodic wifi retry
...
void SetupWiFi()
{
  ...
  for (int i = 0; i < max_clients; i++)
  { // initialise udp remote ip table
    udpremoteip[i][0] = 0;
    udpremoteip[i][1] = 0;
    udpremoteip[i][2] = 0;
    udpremoteip[i][3] = 0;
  ...
  log.print("Read EEPROM apFailover = ");
  log.println(apFailover);
#endif

  //=====================================  S T A T I O N ==========================================

  if ((set.fc_io == fc_wifi) && ((set.gs_io == gs_wifi) || (set.gs_io == gs_wifi_bt)))
  { // if user selects wifi fc uplink AND wifi gcs downlink simultaneously, then wifi mode must be ap_sta
    if (set.wfmode != ap_sta)
    {
  ...
  if ((set.wfmode == sta) || (set.wfmode == ap_sta) || (set.wfmode == sta_ap))
  { // STA mode or AP_STA mode or STA failover to AP mode
    if (!apFailover)
    {
      ...
      while (WiFi.status() != WL_CONNECTED)
      {
        ...
        if (retry > 20)
        {
          ...
          { // STA failover to AP mode
            ...
            EEPROM.commit();
#if defined DEBUG_EEPROM
            log.print("Write EEPROM apFailover = ");
            log.println(apFailover);
#endif
            delay(1000);
            ESP.restart(); // esp32 and esp8266
      ...
      if (WiFi.status() == WL_CONNECTED)
      {
        ...
        if (set.wfmode == sta_ap)
        {                   // in sta failover to ap mode we had successful sta connect
        ...
        { // TCP
          if (set.fc_io == fc_wifi)
          { // if we expect wifi from the fc, we are a client, so need a new session
            outbound_clientGood = newOutboundTCPClient();
          }
        ...
        if ((set.mav_wfproto == udp) || (set.fr_io & 0x02))
        { // UDP

          if (set.wfmode == ap_sta)
          {                                     // in WIFI_AP_STA mode, we need to discriminate between sta and ap read ports
          ...
          if ((set.wfmode == ap_sta) || (set.fc_io == fc_wifi))
          {                                // if wifi both sides OR fc wifi
            udpremoteip[0] = UDP_remoteIP; // [0] IP reserved for FC side
          }
          else if ((set.gs_io == gs_wifi) || (set.gs_io == gs_wifi_bt))
          {                                // if gcs wifi
    ...
    { // if apFailover clear apFailover flag
      apFailover = 0;
      EEPROM.write(0, apFailover); // (addr, val)
      EEPROM.commit();
#if defined DEBUG_EEPROM
      log.print("Clear EEPROM apFailover = ");
      log.println(apFailover);
#endif
    }
  }
  ...
  if ((set.wfmode == ap) || (set.wfmode == sta_ap))
  { // AP mode or STA failover to AP mode
    if (!wifiSuGood)
    { // not already setup in STA above
      ...
      if (set.wfmode == sta_ap)
      {                  // in sta_ap mode we had successful failover to ap
...
void BlinkMavLed(uint32_t period)
{
  ...
  if (cMillis - mav_led_millis >= period)
  { // blink period
...
void OpenSDForWrite()
{
  ...
  if (daylightSaving)
    ap_time_unix_usec -= 3600; // deduct an hour
...
void SP_Byte_To_SD(byte SD_Byte)
{ // FPort to SD
  ...
  { // Sport to SD Card
    ...
    { //  if open for write
      ...
      if (file.write(sd_buf, sd_buf_sz))
      { // write contents of buffer to SD
      }
      else
      {
        log.println("Append failed");
...
uint32_t Get_Volt_Average1(uint16_t mV)
{
  ...
  if (bat1.avg_mV < 1)
    bat1.avg_mV = mV; // Initialise first time
...
{ // in 10*milliamperes (1 = 10 milliampere)
...
uint32_t Get_Volt_Average2(uint16_t mV)
{
  ...
  if (bat2.avg_mV == 0)
    bat2.avg_mV = mV; // Initialise first time
...
uint32_t Get_Current_Average2(uint16_t dA)
{
  ...
  if (bat2.avg_dA == 0)
    bat2.avg_dA = dA; // Initialise first time
...
{ // PWM 1000 to 2000   ->    nominal -63 to 63
  int8_t myint;
...
//=================================================================================================
int8_t THR_To_63(uint16_t value)
{ // THR -100 to 100   ->    nominal -63 to 63
  int8_t myint;
...
uint16_t prep_number(int32_t number, uint8_t digits, uint8_t power)
{
  ...
  if ((digits == 1) && (power == 1))
  { // number encoded on 5 bits: 4 bits for digits + 1 for 10^power
    if (abs_number < 10)
    {
      res = abs_number << 1;
    }
    ...
    { // transmit max possible value (0x0F x 10^1 = 150)
      res = 0x1F;
    }
    if (number < 0)
...
void HandleDisplayButtons()
{
  ...
  if (millis() - last_log_millis > 15000)
  {                             // after 15 seconds default to flight info screen
  ...
  { // if ESP touch pin-pair enumerated
    ...
    if (dnButton)
    {
      ...
      if ((Pup != -1) && (Pdn != -1))
  { // if digital pin-pair enumerated
...
void Scroll_Display(scroll_t up_dn)
{
  ...
  if (millis() - scroll_millis < 300)
    return;
  show_log = true;
  scroll_millis = millis();

  if (up_dn == up)
  { // towards last line painted, so lines move up
    scroll_row--;
  ...
  if (up_dn == down)
  { // towards first line painted, so lines move down
    scroll_row++;
    scroll_row = constrain(scroll_row, scr_h_ch, row);
    dnButton = false;
    PaintLogScreen(scroll_row, show_last_row); // paint down to scroll_row
...
void PaintLogScreen(uint8_t new_row, last_row_t last_row_action)
{
  ...
  { // drop first line, display rest of old lines & leave space for new line
...
void LogScreenPrintln(String S)
{
  ...
  for (col = col; col < scr_w_ch; col++)
  { //  padd out the new line to eol
...
void LogScreenPrint(String S)
{
  ...
  if (col > scr_w_ch - 1)
  { // only if columns exceeded, increment row
    col = 0;
    row++;
  }
  last_log_millis = millis(); // and enable toggle button again
...
void ReadSettingsFromEEPROM()
{
  ...
  log.print("validity_check = ");
  log.println(set.validity_check, HEX);
  log.print("translator mode = ");
  log.println(set.trmode);
  log.print("fr_port_type = ");
  log.println(set.frport);
  log.print("set.fr_io = ");
  log.println(set.fr_io);
  log.print("fc_io = ");
  log.println(set.fc_io);
  log.print("gcs_io = ");
  log.println(set.gs_io);
  log.print("gcs_sd = ");
  log.println(set.gs_sd);
  log.print("sport_sd = ");
  log.println(set.sport_sd);
  log.print("wifi mode = ");
  log.println(set.wfmode);
  log.print("wifi protocol = ");
  log.println(set.mav_wfproto);
  log.print("baud = ");
  log.println(set.baud);
  log.print("wifi channel = ");
  log.println(set.channel);
  log.print("apSSID = ");
  log.println(set.apSSID);
  log.print("apPw = ");
  log.println(set.apPw);
  log.print("staSSID = ");
  log.println(set.staSSID);
  log.print("staPw = ");
  log.println(set.staPw);
  log.print("Host = ");
  log.println(set.host);
  log.print("tcp_localPort = ");
  log.println(set.tcp_localPort);
  log.print("tcp_remotePort = ");
  log.println(set.tcp_remotePort);
  log.print("udp_localPort = ");
  log.println(set.udp_localPort);
  log.print("udp_remotePort = ");
  log.println(set.udp_remotePort);
  log.print("bt mode = ");
  log.println(set.btmode);
  log.print("SlaveConnect To Name = ");
  log.println(set.btConnectToSlave);
  log.print("Major version = ");
  log.println(set.major_version);
  log.print("Minor version = ");
  log.println(set.minor_version);
  log.print("Patch level = ");
  log.println(set.patch_level);
  log.println();
#endif
...
void WriteSettingsToEEPROM()
{
  ...
  log.print("bt mode = ");
  log.println(set.btmode);
  log.print("Master to Slave Name = ");
  log.println(set.btConnectToSlave);
  log.print("Major version = ");
  log.println(set.major_version);
  log.print("Minor version = ");
  ...
  server.on(
    ...
    ESP.restart(); },
      ...
      //   log.setDebugOutput(true);
        ...
        {
          if (Update.end(true))
          { // true to show the percentage progress bar
...
void RawSettingsToStruct()
{
  ...
  else if (FC_Mavlink_IO == 1)
  {
    set.fc_io = fc_bt;
  }
  else if (FC_Mavlink_IO == 2)
  {
    set.fc_io = fc_wifi;
  }
  else if (FC_Mavlink_IO == 3)
  {
    set.fc_io = fc_sd;
  }
  ...
  else if (GCS_Mavlink_IO == 1)
  {
    set.gs_io = gs_bt;
  }
  else if (GCS_Mavlink_IO == 2)
  {
    set.gs_io = gs_wifi;
  }
  else if (GCS_Mavlink_IO == 3)
  {
    set.gs_io = gs_wifi_bt;
  }
  else if (GCS_Mavlink_IO == 9)
  {
    set.gs_io = gs_none;
  }
  ...
  log.print("web_support = ");
  log.println(set.web_support);
  log.print("validity_check = ");
  log.println(set.validity_check, HEX);
  log.print("translator mode = ");
  log.println(set.trmode);
