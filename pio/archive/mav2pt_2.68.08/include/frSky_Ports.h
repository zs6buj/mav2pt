...
#define fpStatusLed -1 // not used in this application
...
void FrSkyPort::initialise()
{
  ...
  for (int i = 0; i < mt20_rows; i++)
  { // initialise MavLite uplink table
    mt20[i].inuse = 0;
  }

  typedef enum wires_set
  {
    one_wire = 1,
    two_wire = 2
  } wire_t;
...
void FrSkyPort::HandleTraffic()
{
  if (frport_id == f_none)
  {
    if (set.frport == f_auto)
    {
      ...
      else if (frport_id == f_port1)
      {
        log.println("FPort1 detected");
      }
      else if (frport_id == f_port2)
      {
...
#if defined TEENSY3X
    if (mavGood && ((millis() - sp_millis) > 1))
    { // very necessary for Teensy 3.x
      sp_millis = millis();
  ...
  else if (frport_id == s_port)
  {
    if (mavGood && ((millis() - sp_millis) > 2))
    { // timing very necessary for S.Port !! especially for Teensy 3.x
      sp_millis = millis();
...
frport_t FrSkyPort::getID_FrPort(uint8_t b)
{
      ...
      if ((prev_b == 0x7E) && (b == 0x1B))
  {
    ...
    if (sp > 10)
    {
      ...
      if (((prev_b == 0x0D) || (prev_b == 0x18) || (prev_b == 0x23)) && (b == 0xff))
  { // fp2 control
    fp2++;
    if (fp2 > 10)
    {
      ...
      if (((prev_b == 0x0D) || (prev_b == 0x18) || (prev_b == 0x23)) && ((b == 0xf1) || (b == 0xf1)))
  { // fp2 OTA
    fp2++;
    if (fp2 > 10)
    {
...
void FrSkyPort::SPort_Read_And_Write(uint8_t *buf)
{
  ...
  { // Receive sensor IDs from X receiver
    ...
    { // Real-Time DIY downlink slot found, slot ours in, and send right now
    ...
    if (byt == 0x7E)
    {
      if ((*(buf + 1) == 0x0D) && (*(buf + 2) == 0x30))
      { // MavLite uplink (to FC) match found
...
void FrSkyPort::FPort_Read_And_Write(uint8_t *buf, frport_t frport_id)
{
  ...
  */
  ...
  while (true)
  { // loop here forever - debugging only
  ...
  { // find start of frame
    ...
    if ((fr_lth == 0x08) || (fr_lth == 0x19))
    { // downlink or control frame
      FrSkyPort::frGood = true;
      FrSkyPort::frGood_millis = millis();
      inject_delay = micros();
...
bool FrSkyPort::ParseFrame(uint8_t *buf, uint16_t lth)
{
  ...
  uint8_t i = 0;
  for (i = 3; i < lth + 2; i++)
  {                              // payload after start byte[0], lth[1] and type[2],
...
bool FrSkyPort::BytesToPWM(uint8_t *buf, int16_t *ch, uint8_t lth)
{ // PWM Channels
...
uint16_t FrSkyPort::PopNextFrame()
{
  ...
  if (sb_oldest_tier1 == 0)
  { // if there are no tier 1 sensor entries
    if (sb_oldest_tier2 > 0)
    {                  // but there are tier 2 entries
  ...
  if (sb_oldest == 0)
    return 0xffff; // flag the scheduler table as empty
  ...
  else if ((FrSkyPort::msg_class_now(sb[idx].msg_id)) == mavlite)
  {
  ...
  if (sb[idx].msg_id < msgid_filter)
  {
    ...
    log.print("  msg_id=0x");
    log.print(sb[idx].msg_id, HEX);
    if (sb[idx].msg_id < 0x1000)
      log.print(" ");
...
void FrSkyPort::Send_Frsky_Frame(uint16_t idx, uint8_t _prime)
{
  ...
  if (set.trmode == ground)
  { // In ground mode we are master and send a diy poll
    if (frport_id == s_port)
    {
    ...
    if (frport_id == f_port1)
    {                                    // FP1 and 2 in ground mode are conjecture right now until I can get a upgraded transmitter to test with
  ...
  else if ((set.trmode == air) || (set.trmode == relay))
  { // In relay mode we are a slave, and only respond
    if (frport_id == f_port1)
    {
      FrSkyPort::SafeWrite(0x08, true); // lth - add crc
      FrSkyPort::SafeWrite(0x81, true); // FP1 frame type
...
bool FrSkyPort::DecodeAndUplinkMavLite()
{
  ...
  if (FrSkyPort::UnchunkMavLitePayload())
  { // when fully expanded

    switch (mt_msg_id)
    { // Decode uplink mavlite messages according to msg-id
    ...
    case 20: //  #20 or 0x14   mavParamRequestRead -
    {
      ...
      if (mt20_row == 0xffff)
        return false; // mt20 table overflowed, ignore this message :(
...
{
  uint16_t i = 0;
  while (mt20[i].inuse == 1)
  { // find empty mt20 uplink row
    //  log.printf("i=%d  mt20[i].inuse=%d\n", i, mt20[i].inuse);

    if (millis() - mt20[i].millis > 5000)
    { // expire the row if no reply from FC in 5 seconds ? bad param_id
      mt20[i].inuse = 0;
...
//===================================================================
void FrSkyPort::PushMessage(uint16_t msg_id, uint8_t sub_id)
{ // Downlink to GCS
  switch (msg_id)
  {
  ...
  case 0xF101: // msg_id 0xF101 RSSI
    ...
    break;
  default:
    log.print("Warning, msg_id ");
    log.print(msg_id, HEX);
    log.println(" unknown");
    break;
  }
}
//===================================================================
void FrSkyPort::PushToEmptyRow(uint16_t msg_id, uint8_t sub_id)
{ // Downlink to GCS
  sb_row = 0;
  while (sb[sb_row].inuse)
  { // find empty F.Port row
    sb_row++;
    if (sb_row >= sb_rows - 1)
    {
  ...
  else if (FrSkyPort::msg_class_now(msg_id) == mavlite)
  {
...
void FrSkyPort::Push_Param_Val_016(uint16_t msg_id)
{ //  0x16 MavLite PARAM_VALUE ( #22 ) Downlink to GCS
#if defined Support_MavLite
  ...
  for (int i = 0; i < mt_totlth; i++)
  {
    if ((mt_payload[i] > 31) && (mt_payload[i] < 127))
      log.write(mt_payload[i]);
  }
  log.println("|");
#endif
...
void FrSkyPort::Push_Command_Ack_04d(uint16_t msg_id)
{ //  0x4d MavLite COMMAND_ACK ( #77 ) Downlink to GCS
#if defined Support_MavLite
...
void FrSkyPort::ChunkMavLitePayload(uint8_t msg_id, uint8_t tlth)
{
  ...
  while (mt_idx <= tlth)
  {
    ...
    if (mt_chunk == 0)
    {
      ...
      // first chunk
      mtf_payload.seq = mt_chunk;
      FrSkyPort::crcStepOut(mtf_payload.lth);
      mtf_payload.lth = mt_paylth;
      FrSkyPort::crcStepOut(mtf_payload.lth); // idx = 1
      mtf_payload.msg_id = msg_id;
      FrSkyPort::crcStepOut(mtf_payload.msg_id); // idx = 0
...
#endif
//===================================================================
void FrSkyPort::Push_Lat_800(uint16_t msg_id)
{                                                      // 0x800
...
//===================================================================
void FrSkyPort::Push_Lon_800(uint16_t msg_id)
{                                                      // 0x800
  pt_gps_status = ap24_fixtype < 3 ? ap24_fixtype : 3; //  0 - 3
  if (pt_gps_status < 3)
    return;
  ...
  log.print("Passthru out LatLon 0x800: ");
  log.print(" ap33_lon=");
  log.print((float)ap33_lon / 1E7, 7);
  log.print(" pt_lon=");
  log.print(pt_lon);
  log.print(" pt_payload=");
  log.print(pt_payload);
...
void FrSkyPort::Push_Text_Chunks_5000(uint16_t msg_id)
{
  ...
  log.print(" pt_severity=");
  log.print(pt_severity);
  log.print(" ");
  log.print(MavSeverity(pt_severity));
  log.print(" Text= ");
  log.print(" |");
  log.print(pt_text);
  log.println("| ");
  ...
  while (pt_chunk_idx <= (pt_txtlth))
  { // send multiple 4 byte (32b) chunks
...
#if defined Frs_Debug_All || defined Frs_Debug_StatusText
    PrintFrPeriod(0);
    log.print(" pt_chunk_num=");
    log.print(pt_chunk_num);
    log.print(" pt_txtlth=");
    log.print(pt_txtlth);
    log.print(" pt_chunk_idx=");
    log.print(pt_chunk_idx);
    log.print(" ");
    strncpy(pt_chunk_print, pt_chunk, 4);
    pt_chunk_print[4] = 0x00;
    log.print(" |");
    log.print(pt_chunk_print);
    log.print("| ");
    log.print(" pt_payload=");
    log.print(pt_payload);
    log.print(" ");
...
void FrSkyPort::Push_AP_status_5001(uint16_t msg_id)
{
  ...
  log.print(" pt_flight_mode=");
  log.print(pt_flight_mode);
  log.print(" pt_simple=");
  log.print(pt_simple);
  log.print(" pt_land_complete=");
  log.print(pt_land_complete);
  log.print(" pt_armed=");
  log.print(pt_armed);
  log.print(" pt_bat_fs=");
  ...
  log.print(" pt_fs=");
  log.print(pt_fs);
  log.print(" pt_fence_present=");
  log.print(pt_fence_present);
  log.print(" pt_fence_breached=");
  log.print(pt_fence_breached);
  log.print(" pt_imu_temp=");
  log.print(pt_imu_temp);
  log.print(" pt_throt=");
  log.print(pt_throt);
  log.print(" pt_payload=");
...
//===================================================================
void FrSkyPort::Push_Bat1_5003(uint16_t msg_id)
{ //  Into F.Port table from #1 SYS_status only
  pt_payload = 0;
  ...
  log.print(" pt_bat1_volts=");
  log.print((float)pt_bat1_volts * 0.1F, 2); // print whole V float
  log.print(" pt_bat1_amps=");
  log.print((float)pt_bat1_amps * 0.1F, 1); // print whole As float
  log.print(" pt_bat1_mAh=");
  log.print(pt_bat1_mAh);
  log.print(" pt_payload=");
  log.print(pt_payload);
  log.print(" ");
...
void FrSkyPort::Push_Home_5004(uint16_t msg_id)
{
  ...
  log.print("pt_home_dist=");
  log.print(pt_home_dist);
  log.print(" pt_home_alt=");
  log.print((float)(pt_home_alt / 10), 1);
  log.print(" az=");
  log.print(az);
  log.print(" pt_home_angle=");
  log.print(pt_home_angle);
  log.print(" pt_home_arrow=");
  log.print(pt_home_arrow); // units of 3 deg
#endif
...
void FrSkyPort::Push_Parameters_5007(uint16_t msg_id, uint8_t sub_id)
{
  ...
  case 1: // Frame type
    ...
    log.print(" pt_param_id=");
    log.print(pt_param_id);
    log.print(" pt_frame_type=");
    log.print(pt_frame_type);
    log.print(" pt_payload=");
    log.print(pt_payload);
    log.print(" ");
  ...
  case 6: // Number of waypoints in mission
    ...
    log.print(" pt_param_id=");
    log.print(pt_param_id);
    log.print(" pt_mission_count=");
    log.println(pt_mission_count);
#endif
...
void FrSkyPort::Push_Bat2_5008(uint16_t msg_id)
{
  ...
  log.print(" pt_bat2_volts=");
  log.print(pt_bat2_volts);
  log.print(" pt_bat2_amps=");
  log.print(pt_bat2_amps);
  log.print(" pt_bat2_mAh=");
  log.print(pt_bat2_mAh);
  log.print(" pt_payload=");
  log.print(pt_payload);
  log.print(" ");
 ...
 */
  ...
  log.print(" pt_ms_seq=");
  log.print(pt_ms_seq);
  log.print(" pt_ms_dist=");
  log.print(pt_ms_dist);
  log.print(" pt_ms_xtrack=");
  log.print(pt_ms_xtrack, 3);
  log.print(" pt_ms_target_bearing=");
  log.print(pt_ms_target_bearing, 0);
  log.print(" pt_ms_cog=");
  log.print(pt_ms_cog, 0);
  log.print(" pt_ms_offset=");
  log.print(pt_ms_offset);
  log.print(" pt_payload=");
  log.print(pt_payload);
  log.print(" ");
...
void FrSkyPort::Push_Terrain_500B(uint16_t msg_id)
{
  ...
  log.print(" ap_current_height=");
  log.print(ap136_current_height);
  log.print(" ap_terrain_spacing=");
  log.print(ap_terrain_spacing);
  log.print(" pt_height_above_terrain=");
  log.print(pt_height_above_terrain);
  log.print(" pt_terrain_unhealthy=");
  log.print(pt_terrain_unhealthy);
  log.print(" pt_payload=");
  log.print(pt_payload);
  log.print(" ");
...
void FrSkyPort::Push_VFR_Hud_50F2(uint16_t msg_id)
{
  ...
  log.print(" pt_air_spd=");
  log.print(pt_air_spd);
  log.print(" pt_throt=");
  log.print(pt_throt);
  log.print(" pt_bar_alt=");
  log.print(pt_bar_alt);
  log.print(" pt_payload=");
  log.print(pt_payload);
  log.print(" ");
...
void FrSkyPort::Push_Rssi_F101(uint16_t msg_id)
{ // msg_id 0xF101 RSSI tell LUA script in Taranis we are connected
  pt_payload = 0;
...
{
  for (int i = 0; i < mt20_rows; i++)
  { // try to find match on waiting param requests
...
void FrSkyPort::PrintPayload(msg_class_t msg_class, telem_direction_t telem_direction)
{
  ...
  else if (telem_direction == downlink)
  {
    in_or_out = '>';
  }
  else
...
void FrSkyPort::PrintMavLiteUplink()
{
  ...
  { // if seq == 0
    ...
    for (int i = 6; i <= 8; i++)
    {
      if ((frbuf[i] > 31) && (frbuf[i] < 127))
        log.write(frbuf[i]); // print ascii
    }
  }
  else
  { // seq > 0
...
{
  if (millis() - FrSkyPort::fpLed_millis >= period)
  { // blink period
...
void FrSkyPort::ReportOnlineStatus()
{
  ...
  if (FrSkyPort::frGood != FrSkyPort::frPrev)
  ...
  if (FrSkyPort::pwmGood != FrSkyPort::pwmPrev)
  { // report on change of status
...
{
  log.print("autoBaud - sensing pin ");
  log.println(s_pin);
  uint8_t i = 0;
  uint8_t col = 0;
  pinMode(s_pin, INPUT);
  ...
  while (gb_baud == 0)
  {
    ...
    {
      log.println();
      log.print("No telemetry found on pin ");
      log.println(s_pin);
      col = 0;
      i = 0;
    }
    ...
    if (t_baud[0] == t_baud[1])
    {
      if (t_baud[1] == t_baud[2])
      {
        if (t_baud[2] == t_baud[3])
        {
          ...
          {
#if defined Debug_All || defined Debug_Baud
            log.print("Consistent baud found=");
            log.println(t_baud[3]);
#endif
            return t_baud[3];
          }
...
uint32_t FrSkyPort::SenseUart(uint8_t s_pin, pol_t pl)
{
  ...
  if (pl == idle_low)
  {
    while (digitalRead(s_pin) == 0)
    {
    }; // idle_low, wait for high bit (low pulse) to start
  }
  else
  {
    while (digitalRead(s_pin) == 1)
    {
    }; // idle_high, wait for high bit (high pulse) to start
  }
  ...
  for (int i = 0; i < 10; i++)
  {
    ...
    if (pl == idle_low)
    { //  Returns the length of the pulse in uS
...
void FrSkyPort::WriteSBUS(uint8_t *buf)
{
  ...
  uint8_t byt = 0;
  while (byt < 25)
  { //  22 bytes plus digi byte = 16 channels plus ch 17 digi channel
