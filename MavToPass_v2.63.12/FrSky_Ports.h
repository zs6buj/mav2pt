...
//=================================================================================================
  ...
  private:  
    ...
    void          SendFrskyFrame(uint8_t sensor_id, uint16_t msg_id);
      ...
      *(buf+2) = FrSkyPort::ReadByte();             // frame_type  
      ...
      if ((fp_lth == 25) || (fp_lth == 8)) {
        ...
        switch(fp_type){
          ...
          case 0x01:      // Uplink Master (their) Frame                  
            FrSkyPort::parseGood = FrSkyPort::ParseFrame(buf, fp_lth); 
            if (FrSkyPort::parseGood) {    
          ...
          case 0x81:      // Downlink Slave Frame
            FrSkyPort::parseGood = FrSkyPort::ParseFrame(buf, fp_lth); 
            if (FrSkyPort::parseGood) {
   ...
   // Log.printf("length=%d\t", length);
    ...
    void FrSkyPort::Send_Frsky_Frame(uint16_t idx) {
      ...
      if (set.trmode == ground) {               // Only if ground mode send these bytes, else XSR sends them
        ...
        FrSkyPort::SafeWrite(0x1B, false);  // alias == instance 
      }
      ...
      if (msg_class == passthru) {                                     
        frame_id = 0x10;                        //  frame_id
        FrSkyPort::SafeWrite(frame_id, true );           
      ...
      if (msg_class == mavlite) {
        frame_id = 0x32;                        //  frame_id   
        FrSkyPort::SafeWrite(frame_id, true );  //  frame_id - MavLite 0x32 
      ...
      if (set.frport == f_port) {    
        FrSkyPort::SafeWrite(0x7E, false);        // STOP_byte - don't stuff or add into crcout               
      }
