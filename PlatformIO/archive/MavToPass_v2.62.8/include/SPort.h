...
{ 
    // Data members 
  public: 
    bool            sb_full = false; // SPort buffer full flag to throttle supply
  private:
  
    bool      printCRC_Out = false;
    byte      nb, lb;                      // NextByt, LastByt of ByteStuff pair     
    int16_t   CRC_Out;                     // CRC of outbound frsky frame
    int16_t   CRC_In;                      // CRC of inbound frsky frame    
  ...
  private:   
    ...
    void          AddCRCOut(uint8_t b);   
    byte          SafeRead(bool isPayload);
    ...
    void SPort::AddCRCOut(uint8_t b) {
       CRC_Out += b;          // add in new byte
       CRC_Out += CRC_Out >> 8;   // add in high byte overflow if any
       CRC_Out &= 0xff;  // mask all but low byte, constrain to 8 bits
       if (printCRC_Out) Debug.printf("AddIn %3d %2X\tCRC_Out_now=%3d %2X\n", b, b, CRC_Out, CRC_Out);
    }
    //=======================================================================  
    ...
    byte SPort::SafeRead(bool isPayload) {
      byte b;  
    ...
    void SPort::SendAndReceive() {  
      ...
      while (frSerial.available())   {  // Receive sensor IDs from X receiver
        Byt =  SPort::SafeRead(false);  // no bytestuff
        if (sp_idx > sp_max -1) {
    ...
    void SPort::Send_SPort_Frame(uint16_t idx) {
      ...
      if (sb[idx].msg_id == 0xF101) {
        ...
        if (set.trmode != relay) {   
          SPort::StuffAndSend(0x7E, false);    // not payload, don't stuff or CRC_Out
      ...
      if (msg_class == mavlite) {
        SPort::SendMavlitePacket(0x1B, sb[idx].msg_id);    // mtf_payload given value in PopNextFrame() above
    ...
    void SPort::SendPassthruPacket(uint8_t sensor_id, uint16_t msg_id) {
      ...
      if (set.trmode == ground) {           // Only if ground mode send these bytes, else XSR sends them
        SPort::StuffAndSend(0x7E, false);       // START/STOP don't stuff or add into CRC_Out
    ...
    void SPort::SendMavlitePacket(uint8_t sensor_id, uint16_t msg_id) {
      ...
      SPort::StuffAndSend(0x32, false );        //  MavLite frame id
    ...
    void SPort::PushMessage(uint16_t msg_id, uint8_t sub_id) {    // Downlink
      if (SPort::sb_full) return;    // Don't push if sport buffer is full
      switch(msg_id) {
    ...
    #if defined Support_MavLite  
      //printCRC_Out = 1; //xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    void SPort::ChunkMavLitePayload(uint8_t msg_id, uint8_t tlth) {
      ...
      //  Debug.printf("mt_idx=%d  tlth=%d   mt_chunk=%d\n", mt_idx, tlth, mt_chunk     ); 
        if (mt_chunk == 0) {
          ...
          // payload starting position  msgid, lth - then 20B payload, then CRC_Out
    
          // first chunk 
          mtf_payload.seq = mt_chunk;      SPort::AddCRCOut(mtf_payload.lth);
          mtf_payload.lth = mt_paylth;     SPort::AddCRCOut(mtf_payload.lth);     // idx = 1          
          mtf_payload.msg_id = msg_id;     SPort::AddCRCOut(mtf_payload.msg_id);  // idx = 0

         //______________________________________________________
         for (int i = 3 ; i < 6 ; i++) {  
            mtf_payload.raw[i] = mt_payload[mt_idx];  
            SPort::AddCRCOut(mtf_payload.raw[i]); 
            #if defined Debug_Mavlite
          ...
          mtf_payload.seq = mt_chunk;  SPort::AddCRCOut(mtf_payload.seq); 
          //_______________________________________________________
          for (int i = 1 ; i < 6 ; i++) { 
          
            if (mt_idx >= tlth) {  // break out of the for loop
              mtf_payload.raw[i] = CRC_Out;     // no 1s complement? and append the CRC_Out
