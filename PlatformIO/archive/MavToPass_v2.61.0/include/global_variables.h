...
//=================================================================================================   
//                              F r S k y   S P o r t   M o d u l e 
//=================================================================================================  

byte     nb, lb;                      // NextByt, LastByt of ByteStuff pair     
//short    CRC;                         // CRC of frsky frame
...
const uint16_t  sp_max = 32;           // low level s.port read buffer
byte            sp_buff[sp_max];
uint16_t        sp_idx = 0;

// ========================== MavLite Whole Message

   typedef union {
   struct {
          uint8_t     msg_id;       //  1B         
          uint8_t     lth;          //  1B    Payload length
          uint16_t    packing;      //  2B    4B word boundary  
              
          float       value;        //  4B   |payload 20B
          char        param_id[16]; // 16B   |
          
          uint8_t     crc;          //  1B     
          };    
         uint8_t      raw[25];      // 25B - total 0 thru 24
          }
  mavlite_msg_t;  
  
  mavlite_msg_t       mliteMsg; 
  
  uint8_t   msg_idx = 0; 
  uint8_t   mtmsg_idx = 2; 
  
// ========================== MavLite Payload Frame (chunk)     
   typedef union {
   struct {
          uint8_t seq;       // 1B
          uint8_t lth;       // 1B           
  ...
  mt_payload_t  mt_Payload;

 volatile uint8_t mt_chunk = 0;
 volatile bool    mt_end = false;
// ========================== MavLite Param Request Table
// MavLite Param-Request_Read #20 (0x14)    READ
  
  typedef struct  {            // table of param requests waiting for replies from FC   
    uint8_t    msg_id;         // 1B 
    char       param_id[16];   // 16B
    uint32_t   millis;         // 4B
    uint8_t    inuse;          // 1B
...
// ========================== MavLite Param Request
// MavLite Param-Request_Read #22 (0x16)    Downlink
  uint8_t mt22_lth;           // payload lth
  uint8_t mt22_tot;           // payload lth plus 4
  
//==========================
  uint8_t   mt_sens_id;
  uint8_t   mt_payload_id;
  uint8_t   mt_seq;
  uint8_t   mt_paylth;
  uint8_t   mt_msg_id;
  char      mt_param_id[16];
...
//                            S  C  H  E  D  U  L  E  R
//
//===============  Combined Mavlink and MavLite scheduler buffer ========================
  ...
  uint8_t       mt_inuse[mt_rows];            // 1B

  uint32_t        pt_payload[sb_rows];          // 4B Passthru payload buffer
  mt_payload_t    mt_payload[mt_rows];          // 6B MavLite payload buffer
