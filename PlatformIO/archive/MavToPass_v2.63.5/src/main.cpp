...
//================================================================================================= 
    ...
    void HandleDisplayButtons() {
      ...
      if (millis() - last_log_millis > 20000) { // after 20 seconds default to flight info screen
    ...
    void DisplayFlightInfo() {
        ...
        if (millis() - info_millis > 200) {    // refresh rate
          ...
          yy = 16 * CHAR_W_PX;        
          display.setCursor(xx, yy); 
          snprintf(snprintf_buf, snp_max, "mAh:%d", pt_bat1_mAh);     
          display.fillRect(xx+(4*CHAR_W_PX), yy, (5*CHAR_W_PX), CHAR_H_PX, ILI9341_BLUE); // clear the previous line   
          display.println(snprintf_buf);           
        ...
        if (millis() - info_millis > 2000) {    // refresh rate
          ...
          display.setCursor(0, 2 * CHAR_H_PX);   //x , y   
          snprintf(snprintf_buf, snp_max, "Lon %.7f", cur.lon);
          display.println(snprintf_buf);  
      
          display.setCursor(0, 4 * CHAR_H_PX);   //x , y    
          snprintf(snprintf_buf, snp_max, "%.1fV %.1fA %dmAh", (float)ap_voltage_battery1/1000, (float)ap_current_battery1/100, pt_bat1_mAh);     
          display.println(snprintf_buf);  
      
          display.setCursor(0,6 * CHAR_H_PX);   //x , y    1 line = 8 pixels
...
//=================================================================================================
    ...
    uint32_t draw_horizon(float roll, float pitch, int16_t width, int16_t height) {
      ...
      roll_slope = tan(roll * PI / 180);     // roll slope 
      pitch_offset = (sin(pitch * PI / 180)) * yc;    
      tick_lean = roll_slope * tick_height;

      lth = (xc * 0.8) * cos(roll * PI / 180);
      x0 = (xc - lth);
      x1 = (xc + lth);
