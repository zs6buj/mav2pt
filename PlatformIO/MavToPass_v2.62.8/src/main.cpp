...
void loop() {
 ...
 //====================
  ...
  #if defined Debug_Loop_Period   // For debugging
    PrintLoopPeriod();
  #endif
 ...
 // if (millis() - downlink_millis > 1) {   
    RB_To_Decode_and_GCS();              // main timing loop for mavlink decode and to GCS
 // }
...
void RB_To_Decode_and_GCS() {
  ...
  if (!MavRingBuff.isEmpty()) {
    ...
    DecodeOneMavFrame();  // Decode a Mavlink frame and buffer passthru frames
  }
}  
...
uint16_t mav_len;                // Length of payload
 ...
 */
  ...
  if (mavNum == 1) {
    ...
    if (j == 0) {
      PrintByte(bytes[0], 0);   // CRC_Out1
      PrintByte(bytes[1], 0);   // CRC_Out2
      Debug.print("/");
      }
    mav_magic = bytes[j+2];   
    mav_len = bytes[j+3];
