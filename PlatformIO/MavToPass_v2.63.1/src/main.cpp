...
//=================================================================================================
  ...
  // =============================================
    ...
    if (set.frport == f_port) {
     Debug.println("F.Port selected");
     DisplayPrintln("F.Port selected");
    } else  
    if (set.frport == s_port) {
      Debug.println("S.Port selected");
     DisplayPrintln("S.Port selected");
    } else  
    if (set.frport == f_none) {
      Debug.println("No FrSky port selected");
      DisplayPrintln("No FrSky support");
    }

    if (set.trmode == ground) {
     Debug.println("Ground Mode selected");
      DisplayPrintln("Ground Mode");
   } else  
   if (set.trmode == air) {
      Debug.println("Air Mode selected");
      DisplayPrintln("Air Mode");
   } else
   if (set.trmode == relay) {
      Debug.println("Relay Mode selected");
     ...
     DisplayPrintln("Setup mAh in LUA");     
    #else
  ...
  #else
      Debug.println("No FrSky port support selected or built in");
      DisplayPrintln("No FrSky support");
  #endif
  ...
  if ((set.fc_io == fc_wifi) || (set.gs_io == gs_wifi) ||  (set.gs_io == gs_wifi_bt) || (set.web_support)) {
   if (set.wfmode == ap)  {
     ...
     DisplayPrintln("WiFi-mode=STA/AP");
   }
    
   if (set.wfproto == tcp)  {
     Debug.println("Protocol is TCP/IP");
     DisplayPrintln("Protocol=TCP/IP");
   }
   else if  (set.wfproto == udp) {
     Debug.print("Protocol is UDP");
     DisplayPrintln("Protocol = UDP");
      ...
      if ((set.wfproto == tcp) && (inbound_clientGood))  { // TCP from GCS side    
        ...
        if (msgReceived) {
          GCS_available = true;  // Record waiting to go to FC 
            ...
            Debug.print("Read WiFi TCP to G2Fmsg: msgReceived=" ); Debug.println(msgReceived);
            if (msgReceived) PrintMavBuffer(&G2Fmsg);
          #endif      
        }
      }
      ...
      if (set.wfproto == udp)  { // UDP from GCS
        ...
        if (msgReceived) {
          GCS_available = true;  // Record waiting to go to FC 
            ...
            Debug.print("Read WiFi UDP to G2Fmsg: msgReceived=" ); Debug.println(msgReceived);
            if (msgReceived) PrintMavBuffer(&G2Fmsg);
          #endif      
        }   
      } 
    ...
    switch(G2Fmsg.msgid) {
       case MAVLINK_MSG_ID_HEARTBEAT:    // #0
          ...
          gshbGood = true;
            ...
            Debug.print("=====>GCS to FC: #0 Heartbeat: ");  
            Debug.print("gcs_sysid="); Debug.print(ap_sysid);   
            Debug.print("  gcs_compid="); Debug.print(ap_compid);                     
        ...
        case MAVLINK_MSG_ID_PARAM_REQUEST_READ:  // #20  from GCS
            ...
            Debug.print("=====>GCS to FC: #20 Param_Request_Read: ");           
        ...
        case MAVLINK_MSG_ID_PARAM_REQUEST_LIST:  // #21  To FC from GCS
            ...
            Debug.print("=====>GCS to FC: #21 Param_Request_List: ");           
            Debug.print("gcs_target_system="); Debug.print(gcs_target_system); 
         ...
         case MAVLINK_MSG_ID_MISSION_REQUEST_INT:  // #51 To FC from GCS
            ...
            Debug.print("=====>GCS to FC: #51 Mission_Request_Int: ");           
            Debug.print("gcs_target_system="); Debug.print(gcs_target_system);   
            Debug.print("  gcs_target_component="); Debug.print(gcs_target_component);          
...
void MavToRingBuffer() {
      ...
      if (MavRingBuff.isFull()) {
        BufLedState = HIGH;
        Debug.println("MavRingBuff full. Ignoring new records");
     //   DisplayPrintln("Mav buff full"); 
      }
       else {
        BufLedState = LOW;
    ...
    if ((set.gs_io == gs_wifi) || (set.gs_io == gs_wifi_bt)) { //  WiFi
      ...
      if (wifiSuGood) {
          ...
          for (int i = 0 ; i < max_clients ; ++i) {       // send to each active client
            if (NULL != clients[i]) {        // if active client
              ...
              if ((millis() -  prev_millis) > 4) {
                PrintPeriod(false);  Debug.println(" TCP Write timeout =====================================>");  
              }
              
              #ifdef  Debug_GCS_Down
                if (msgSent) {
                  Debug.printf("Sent to GCS client %d by WiFi TCP: msgid=%d\n", active_client_idx+1, R2Gmsg.msgid);  
                }
              //  Debug.printf("Sent to GCS client %d by WiFi TCP: msgid=%d  msgSent=%d\n", active_client_idx+1, R2Gmsg.msgid, msgSent);
              //  PrintMavBuffer(&R2Gmsg);
              #endif
            }
          }    
        ...
        if (set.wfproto == udp)  { // UDP               
         ...
         #endif  
         #ifdef  Debug_GCS_Down
           Debug.print("Sent to GCS by WiFi UDP: msgSent="); Debug.println(msgSent);
           PrintMavBuffer(&R2Gmsg);
         #endif         
        }                                                                     
      }  
   ...
   switch(R2Gmsg.msgid) {
        ...
        case MAVLINK_MSG_ID_HEARTBEAT:    // #0   https://mavlink.io/en/messages/common.html
          ...
          if(!mavGood) {
            ...
            Debug.print("ap_sysid="); Debug.print(ap_sysid);   
            Debug.print("  ap_compid="); Debug.print(ap_compid);                      
   ...
   if ((set.wfmode == ap) || (set.wfmode == sta_ap)) { // AP mode or STA failover to AP mode
    ...
    wifiSuGood = true;  
     ...
     if (wifiSuGood) {
       ...
       DisplayPrintln("webSupprt active");  
     }  else {
...
//================================================================================================= 
    ...
    void PaintDisplay(uint8_t new_row, last_row_t last_row_action) {   
        ...
        #if (defined ST7789_Display) || (defined SSD1331_Display)
          display.fillScreen(screenBackground);                 
...
//=================================================================================================
    ...
    void Free_Bluetooth_RAM() {
      ...
      if (!btDisabled) {  // if not already disabled           
        ...
        btActive = false;
        btDisabled = true;
        Debug.println("Bluetooth disabled to free up SRAM for web support. PROCEED TO REBOOT for BT to be restored"); 
...
void RecoverSettingsFromFlash() {
  ...
  if (set.validity_check != 0xdc) {            // eeprom does not contain previously stored setings
    ...
    Debug.println("NOTE! ALL SETTINGS IN EEPROM SET TO COMPILE_TIME DEFAULTS!");                                           
...
//=================================================================================
  ...
  settingsPage  = styleSettings;
    ...
    sprintf(temp, "<input type='radio' class='big' name='_frport' value='None' %s> None &nbsp &nbsp", set.frport0);
    settingsPage += temp;      
    sprintf(temp, "<input type='radio' class='big' name='_frport' value='SPort' %s> SPort &nbsp &nbsp ", set.frport1);
    settingsPage += temp;
    sprintf(temp, "<input type='radio' class='big' name='_frport' value='FPort' %s> FPort <br>", set.frport2);
    settingsPage += temp;  
  ...
  sprintf(temp, "<input type='radio' onclick=checkserial(this.form) class='big' name='_fc_io' value='Serial' %s> Serial &nbsp &nbsp", set.fc_io0);
  settingsPage += temp;
  ...
  #endif  
  #if defined sdBuiltin   
    settingsPage += " <br> DOWNLINK SD: &nbsp";
  ...
  settingsPage += temp;
  sprintf(temp, "TCP Local Port: <input type='text' name='_tcp_localPort' value='%d' size='2' minlength='2' required maxlength='5'> <br>", set.tcp_localPort);
  settingsPage += temp;
  sprintf(temp, "UDP Local Port: <input type='text' name='_udp_localPort' value='%d' size='2' minlength='2' required maxlength='5'> <br>", set.udp_localPort);
  settingsPage += temp;
  sprintf(temp, "UDP Remote Port: <input type='text' name='_udp_remotePort' value='%d' size='2' minlength='2' required maxlength='5'> <br>", set.udp_remotePort);
  settingsPage += temp;
...
byte b;
    ...
    if(strlen(set.apPw) < 8) {
      ...
      Debug.println("AP Password < 8 chars long! Will fail to authenticate");
    ...
    b = EEPROMRead8(162);                         // fr-port type 162
    if (b == 0) {
      set.frport = f_none;
    } else if (b == 1) {
      set.frport = s_port;
    ...
    RefreshHTMLButtons();     
      ...
      Debug.print("sport_sd = "); Debug.println(set.sport_sd);            
      ...
      Debug.print("fr_port_type = "); Debug.println(set.frport);
      Debug.println();          
    #endif  
...
void WriteSettingsToEEPROM() {
      ...
      EEPROMWrite8(162, set.frport);                 // 162       
      EEPROM.commit();  
      RefreshHTMLButtons();
        ...
        Debug.print("frport = "); Debug.println(set.frport);         
        Debug.println();             
      #endif 
}  
//=================================================================================
void ReadSettingsFromForm() {
  ...
  if (S == "Serial") {
    if (set.fc_io == fc_ser) {
      Debug.println("Selection of simultaneous serial downlink and serial uplink ignored!");
      DisplayPrintln("Bad serial selection!");     
    } else {
      set.gs_io = gs_ser;
    }
  } else
  ...
  strcpy(set.btConnectToSlave, S.c_str());    // strcpy() copies the C string including the terminating null character
   ...
   if (S == "FPort") {
      set.frport = f_port;
        ...
        Debug.print("bt mode = "); Debug.println(set.btmode); 
        Debug.print("Master to Slave Name = "); Debug.println(set.btConnectToSlave);  
        Debug.print("frport = "); Debug.println(set.frpo);        
        Debug.println();      
      #endif 
 }
//=================================================================================
