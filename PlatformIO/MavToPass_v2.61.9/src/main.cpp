...
void loop() {            // For WiFi STA only
    ...
    if ((set.gs_io == gs_wifi) || (set.gs_io == gs_wifi_bt)) {  // Only when we expect incoming remote clients
      ...
      if (set.wfproto == tcp)  {  // TCP  
        if (wifiSuGood) {
          TCPclient = TCPserver.available();              // we are a server, so listen for inbound clients 
          if(TCPclient) {
            Debug.println("New inbound remote tcp client connected"); 
    ...
    if ((set.gs_io == gs_wifi) || (set.gs_io == gs_wifi_bt)) { //  WiFi
      ...
      if (wifiSuGood) {
        ...
        if (set.wfproto == tcp)  { // TCP  
          ...
          #ifdef  Debug_GCS_Down
             Debug.print("Passed down from Ring buffer to GCS by WiFi TCP: msgSent="); Debug.println(msgSent);
            PrintMavBuffer(&R2Gmsg);
          #endif
        }
...
//=================================================================================================  
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        if (set.wfproto == tcp)  {   // TCP
          ...
          if (set.fc_io == fc_wifi) {  // if we expect wifi from the fc, we are a client, so connect ..
            while (!TCPclient.connect(TCP_remoteIP, TCP_remotePort)) {
        ...
        if (set.wfproto == udp)  {  // UDP
          ...
          Debug.printf("UDP started, local IP %s, port %d \n", localIP.toString().c_str(), set.udp_localPort);
  ...
  if ((set.wfmode == ap) || (set.wfmode == sta_ap)) { // AP mode or STA failover to AP mode
    if (!wifiSuGood) {  // not already setup in STA above  
      ...
      WiFi.softAP(set.apSSID, set.apPw);
  ...
  //=================================================================================================  
   void PrintRemoteIP() {
    if (FtRemIP)  {
      FtRemIP = false;
      Debug.print("UDP client connected, remote IP: "); Debug.print(UDP_remoteIP);
