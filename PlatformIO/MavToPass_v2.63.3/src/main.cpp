...
//=================================================================================================
    ...
    Log.println("Display support activated");
  ...
  //===============================       H a n d l e   W i F i   E v e n t s 
   ...
   if ((set.wfmode == sta) || (set.wfmode == sta_ap))  {  // STA mode or STA failover to AP mode
    if (!apFailover) {   
      ...
      if (WiFi.mode(WIFI_STA)) {
         Log.println("Wi-Fi mode set to STA sucessfully");  
      } else {
        Log.println("Wi-Fi mode set to STA failed!");  
      }
   ...
   if ((set.wfmode == ap) || (set.wfmode == sta_ap)) { // AP mode or STA failover to AP mode
    if (!wifiSuGood) {  // not already setup in STA above  
      ...
      Log.printf("Wi-Fi mode set to WIFI_AP %s\n", WiFi.mode(WIFI_AP) ? "" : "Failed!"); 
...
//================================================================================================= 
    ...
    void HandleDisplayButtons() {
      ...
      static bool infoPressBusy = false;
      static bool infoNewPress = false;         
      static bool infoPressed = false;
      static bool show_log = true;    
      static uint32_t info_debounce_millis = 0; 
      static uint32_t  info_millis = 0; 
      static uint32_t  last_log_millis = 0;
      static const uint16_t db_period = 1000; // debounce period mS
           
    #if ((defined ESP32) || (defined ESP8266))   // esp8266 digital pin interrupt is momentary
         
      // if touch pin for info display enumerated infoButton => true 
      ...
      if (show_log) {
        ...
        if (millis() - info_millis >= 2000) { // refresh every 2 sec 
      ...
      if ( (Tup != 99) && (Tdn != 99) ) {   // if touch pin-pair enumerated
        if (upButton) {
          Scroll_Display(up);
        }
        if (dnButton) {
    ...
    void DisplayFlightInfo() {
      ...
      display.setCursor(0, 2 * char_height);   //x , y   
      snprintf(snprintf_buf, snp_max, "Lon %.7f", cur.lon);
      display.println(snprintf_buf);  
      
      display.setCursor(0, 4 * char_height);   //x , y    
      snprintf(snprintf_buf, snp_max, "%.1fV %.1fA %dmAh", (float)ap_voltage_battery1/1000, (float)ap_current_battery1/100, pt_bat1_mAh);     
      display.println(snprintf_buf);  
      
      display.setCursor(0,6 * char_height);   //x , y    1 line = 8 pixels
      snprintf(snprintf_buf, snp_max, "Sats %d RSSI %d%%", ap24_sat_visible, pt_rssi);     
      display.println(snprintf_buf);       
...
void ReadSettingsFromForm() {
  ...
  if (S == "Serial") {
    if (set.fc_io == fc_ser) {
      Log.println("Selection of simultaneous serial downlink and serial uplink ignored!");
  ...
  strcpy(set.btConnectToSlave, S.c_str());    // strcpy() copies the C string including the terminating null character
   ...
   if (S == "FPort") {
      set.frport = f_port;
        ...
        Log.print("bt mode = "); Log.println(set.btmode); 
        Log.print("Master to Slave Name = "); Log.println(set.btConnectToSlave);  
        Log.print("frport = "); Log.println(set.frpo);        
        Log.println();      
      #endif 
 }
//=================================================================================
