...
void FrSkySPort_Init(void)  {
...
#if (defined ESP32)
   Debug.print("set.trmode = "); Debug.println(set.trmode);
   set.trmode = ground;
  if (set.trmode == ground) {
    frInvert = true;
    ...
    Debug.println("ESP32 S.Port pins NOT inverted for Air or Relay Modes. Must use a hw converter");  
  }
  ...
  */
  ...
  #if defined ESP8266_Invert_SPort_To_Onewire
    frInvert = true;
    frRx = -1;
    frTx = Fr_txPin;
    Debug.println("ESP8266 S.Port txpin inverted to single wire");  
  #else
    Debug.println("ESP8266 S.Port txpin and rx pins need an inverter/single-wire converter");  
  #endif

  frSerial.begin(frBaud, SWSERIAL_8N1, frRx, frTx, frInvert);     

#else   // all onon-esp boards
...
void RawSettingsToStruct() {
  ...
  strcpy(set.btSlaveConnectTo, BT_ConnectToName);                
